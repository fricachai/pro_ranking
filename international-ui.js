...
function renderInternationalContext(c) {
  ...
  const regime = c.summary?.regime || '資料不足';
  const easyDirection = regime === '環境較穩'
    ? { tone: 'support', label: '環境較穩', entry: '尚未持有：可找符合條件個股，分批、不追價。', holding: '已經持有：可續抱，但仍看個股價格與法人是否轉弱。', why: '現在是「可以找機會」，不是「看到上漲就買」。' }
    : regime === '風險升溫'
      ? { tone: 'caution', label: '先保守', entry: '尚未持有：先不要急著買，等風險降溫與個股條件確認。', holding: '已經持有：先檢查個股是否跌破自己的風險條件。', why: '現在先保護資金，再等待較清楚的買點。' }
      : regime === '資料不足'
        ? { tone: 'blocked', label: '資料不足', entry: '尚未持有：先不要買，等資料補齊。', holding: '已經持有：先不要只看這一區做賣出決定。', why: '資料不完整時，任何方向都不應被當成確定答案。' }
        : { tone: 'mixed', label: '先觀望', entry: '尚未持有：先不要急著買，只找價格與法人都確認的個股。', holding: '已經持有：不因單一國際指標賣出，先看個股風險條件。', why: '目前訊號沒有站在同一邊，先等個股條件比猜大盤更重要。' };
  ...
  const marketJudgment = regime === '風險升溫' ? '先不要新增部位；已有持股先看個股是否破壞。' : regime === '環境較穩' ? '可以找個股，但仍要等價格承接，不追價。' : '先不要因美股或VIX單獨買賣，回到個股條件。';
  const industryJudgment = Number.isFinite(sox5) && sox5 > 0.5 ? '電子股可列入觀察；能源股還要看油價對成本的影響。' : '半導體方向不夠強，電子與能源股都先看個股條件。';
  const flowJudgment = '不要看到淨空就直接賣股；先確認臺股價格與外資現貨是否一起轉弱。';
  ...
  const cards = [
     card('sp500', '美股 S&P 500', n(d.sp500?.value, 0), `20筆 ${s(d.sp500?.change20)}｜5筆 ${s(d.sp500?.change5)}`, d.sp500, '', '它代表全球市場氣氛，不是臺股個股答案。', trendJudgment('sp500', d.sp500?.change20, '目前建議：可找符合條件個股，但不要追價', '目前建議：先觀望，不新增部位', '目前建議：先不因美股買賣，回看個股條件', 0.5)),
     card('fx', '美元／臺幣', n(d.fx?.usdTwd?.value, 3), `5筆 ${s(d.fx?.usdTwd?.change5)}｜上升＝臺幣貶值`, d.fx?.usdTwd, '期交所參考值，非銀行成交報價', '臺幣貶值常代表資金壓力增加，但不能單獨判定賣出。', trendJudgment('fx', d.fx?.usdTwd?.change5, '目前建議：可回看個股條件，仍不追價', '目前建議：先觀望，不新增部位；持股不因這一項單獨賣出', '目前建議：先不因匯率買進或賣出，回看外資與個股價格', 0.5)),
     card('treasury', '美國10年債殖利率', `${n(d.treasury?.value, 2)}%`, `5筆 ${s(Number.isFinite(d.treasury?.difference5) ? d.treasury.difference5 * 100 : null, 0, '基點')}｜2年 ${n(d.treasury?.y2, 2)}%`, d.treasury, '', '殖利率上升會增加部分股票的估值壓力，不等於所有股票都要賣。', trendJudgment('treasury', d.treasury?.difference5, '目前建議：可找符合條件個股，檢查估值後再承接', '目前建議：成長股先觀望，等待價格結構確認', '目前建議：先不因利率買賣，回看個股估值與趨勢', 0.2)),
     card('vix', 'VIX 波動指數', n(d.vix?.value, 2), '低於20／高於25為觀察門檻，非買賣訊號', d.vix, '', '它代表市場緊張程度；高檔先保守，不代表持股必須賣出。', judgment('vix', Number.isFinite(d.vix?.value) && d.vix.value >= 25 ? '目前建議：先觀望，不新增部位；持股先看個股破壞條件' : Number.isFinite(d.vix?.value) && d.vix.value < 20 ? '目前建議：環境較穩，可找符合條件個股，但仍不追價' : '目前建議：先不因波動指數買賣，等待其他條件確認')),
     card('tx', '外資臺指期淨部位', `${n(d.tx?.net, 0)}口`, d.tx?.comparisonDate ? `較 ${d.tx.comparisonDate} ${s(d.tx.changeFromPrevious, 0, '口')}` : '首筆快照：尚無前期比較', null, '負值＝淨空；可能包含避險，不能單獨判空', '先看它是否與個股價格同向；淨空單不等於外資正在賣股票。', judgment('tx', '目前建議：不可單獨買賣；只作確認，回看個股價格與法人條件')),
     card('dollar', 'Fed 廣義美元', n(d.dollar?.value, 2), `公布期間 ${s(d.dollar?.periodChange)}｜${d.dollar?.periodStart || '—'} 起`, d.dollar, '非 ICE DXY；週資料有發布落差', '它是全球資金背景，不是臺股的直接買賣訊號。', trendJudgment('dollar', d.dollar?.periodChange, '目前建議：資金環境較緊，先觀望新部位', '目前建議：可回看符合條件個股，仍不追價', '目前建議：先不因美元買賣，回看個股條件', 0.5))
   ].join('');
  ...
  return `<section class="section international-context" id="internationalContext" data-context-version="${e(c.version)}">
     <div class="context-heading"><div><h2>國際市場脈動</h2><p>先看環境，再看個股觸發條件。</p></div><a href="#quickGuide">直接查個股 ↓</a></div>
      ...
      <div class="context-judgment-key"><b>簡單判讀怎麼看：</b><span>「可找個股」＝環境沒有明顯阻力；「先觀望」＝暫不新增部位；「提高警戒」＝先檢查持股；「不可單獨買賣」＝這項資料只能當輔助。</span></div>
      <div class="context-easy-direction context-easy-${e(easyDirection.tone)}"><div class="context-easy-title"><span>給不熟投資的人</span><strong>${e(easyDirection.label)}</strong></div><p class="context-easy-why">${e(easyDirection.why)}</p><div class="context-easy-actions"><div><b>尚未持有</b><span>${e(easyDirection.entry)}</span></div><div><b>已經持有</b><span>${e(easyDirection.holding)}</span></div></div><small>這是國際環境的方向提示；真正買賣仍以個股卡片的價格、法人、基本面與風險條件為準。</small></div>
    ...
    <div class="context-refresh"><span>國際資料查詢：${e(localTime(c.checkedAt))}（臺北）${weak ? ` · ${weak}項落後／缺漏` : ''}</span><button id="checkPublishedUpdate" type="button">查看最新發布</button><span id="publishedUpdateStatus" role="status" aria-live="polite"></span></div>
     <details class="context-details"><summary>半導體、能源與國際法人部位</summary><div class="context-section-guide"><b>這一組回答：哪些產業或資金方向值得進一步核對？</b><span>它用來確認臺股個股的產業順風、成本壓力與法人情緒，不取代個股基本面，也不直接產生買賣訊號。</span></div><div class="context-grid context-grid-extra">
        ${card('sox', '費城半導體', n(d.sox?.value, 0), `5筆 ${s(d.sox?.change5)}｜相對S&P ${s(soxRelative, 1, '百分點')}`, d.sox, '', '確認電子／半導體個股是否有產業面配合；只作順風或逆風檢查', trendJudgment('sox', soxRelative, '目前建議：電子股可列入觀察，等個股承接再買', '目前建議：電子股新部位先觀望', '目前建議：先不因半導體買賣，回看個股趨勢', 0.5))}
        ${card('oil', 'WTI近月期貨', `${n(d.oil?.value, 2)}美元／桶`, `5筆 ${s(d.oil?.change5)}｜注意合約轉倉`, d.oil, '', '檢查能源、運輸與製造成本壓力；先回看個股受益或受害方向', trendJudgment('oil', d.oil?.change5, '目前建議：成本受影響產業先觀望；能源股仍須看個股', '目前建議：成本壓力減輕，可回看受益個股', '目前建議：先不因油價買賣，回看個股產業位置', 2))}
        ${card('fx', '亞洲匯率參考', `USD/JPY ${n(d.fx?.usdJpy?.value, 2)}`, `USD/CNY ${n(d.fx?.usdCny?.value, 4)}｜均為一美元兌本幣`, null, '', '補充出口、進口與區域資金背景；不能單獨判定臺股漲跌', judgment('fx', '目前建議：只作背景參考，不要依這張圖單獨買或賣'))}
...
const styles = `
 ...
 .context-easy-direction{border:2px solid #397866;border-radius:12px;background:#f3f8f4;padding:14px 16px;margin:12px 0 14px}.context-easy-caution{border-color:#b77927;background:#fff8e8}.context-easy-blocked{border-color:#8b5b5b;background:#fff4f2}.context-easy-mixed{border-color:#b77927;background:#fffaf0}.context-easy-title{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.context-easy-title span{font-size:13px;color:#315b49}.context-easy-title strong{font-size:24px;color:#174f3a}.context-easy-caution .context-easy-title strong,.context-easy-mixed .context-easy-title strong{color:#7b531b}.context-easy-blocked .context-easy-title strong{color:#744545}.context-easy-why{margin:6px 0 10px;font-size:14px;font-weight:650}.context-easy-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.context-easy-actions>div{display:flex;flex-direction:column;gap:3px;background:#fff;border:1px solid #d7e1da;border-radius:8px;padding:9px 11px}.context-easy-actions b{font-size:12px;color:#174f3a}.context-easy-actions span,.context-easy-direction>small{font-size:13px;line-height:1.55}.context-easy-direction>small{display:block;margin-top:9px;color:#59665f}
