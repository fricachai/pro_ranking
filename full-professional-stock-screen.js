...
const TODAY = new Intl.DateTimeFormat('en-CA', {
  ...
  const SOURCES = {
    ...
    tpexMargin: 'https://www.tpex.org.tw/openapi/v1/mopsfin_187ap17_O',
    tpexForeignHolding: 'https://www.tpex.org.tw/openapi/v1/tpex_3insti_qfii',
    tpexInstitutional: 'https://www.tpex.org.tw/openapi/v1/tpex_3insti_daily_trading',
    tpexMarginTrading: 'https://www.tpex.org.tw/openapi/v1/tpex_mainboard_margin_balance',
    twseSecurities: 'https://openapi.twse.com.tw/v1/opendata/t187ap03_L',
    tpexSecurities: 'https://www.tpex.org.tw/openapi/v1/tpex_securities',
...
const HORIZON_MODELS = Object.freeze({
  short: {
    label: '短期時機',
    weights: { technicalTiming: 30, ownershipFlow: 25, eventCatalyst: 15, riskLiquidity: 15, fundamentalGuardrail: 15 }
  },
  medium: {
    label: '中期研究',
    primary: true,
    weights: { earningsTrend: 25, businessQuality: 20, technical: 20, valuation: 15, ownership: 10, eventRisk: 10 }
  },
  long: {
    label: '長期初篩',
    preliminary: true,
    weights: { businessQuality: 25, financialResilience: 20, growthDurability: 15, valuation: 25, capitalAllocation: 15 },
    scoredWeight: 85,
    missingWeight: 15
  }
});

const DATA_HEALTH_HARD_GATE = 65;
// 原始事件檔可保留內部人異動以維持來源契約；公開報告與投資判讀一律排除。
const PUBLIC_EXCLUDED_EVENT_TYPES = new Set(['insider_transfer']);

const BALANCE_ENDPOINTS = {
  TWSE: ['ci', 'fh', 'basi', 'ins', 'mim', 'bd'].map(type => `https://openapi.twse.com.tw/v1/opendata/t187ap07_L_${type}`),
  TPEX: ['ci', 'fh', 'basi', 'ins', 'mim', 'bd'].map(type => `https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap07_O_${type}`)
};

const INCOME_ENDPOINTS = {
  TWSE: ['ci', 'fh', 'basi', 'ins', 'mim', 'bd'].map(type => `https://openapi.twse.com.tw/v1/opendata/t187ap06_L_${type}`),
  TPEX: ['ci', 'fh', 'basi', 'ins', 'mim', 'bd'].map(type => `https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap06_O_${type}`)
};
...
function roundToTwseTick(price, direction = 'nearest') {
  const tick = twseTick(price);
  if (!tick) return null;
  const scaled = price / tick;
  const rounded = direction === 'down' ? Math.floor(scaled) : direction === 'up' ? Math.ceil(scaled) : Math.round(scaled);
  return round(rounded * tick, tick < 0.1 ? 2 : tick < 1 ? 1 : 0);
}

function triggerPriceZone(price) {
  if (!Number.isFinite(price) || price <= 0) return null;
  const halfWidth = Math.max(price * 0.005, (twseTick(price) || 0) * 2);
  return {
    low: roundToTwseTick(price - halfWidth, 'down'),
    high: roundToTwseTick(price + halfWidth, 'up'),
    center: roundToTwseTick(price)
  };
}
...
async function fetchYahooOhlc(code, market = 'TWSE') {
  const suffix = market === 'TPEX' ? '.TWO' : '.TW';
  const url = `${SOURCES.yahooChart}${encodeURIComponent(code)}${suffix}?range=6mo&interval=1d&events=div%2Csplits`;
...
function tpexForeignHoldingFeatures(row, fallbackDate) {
  const shares = number(row?.['CurrentlySharesOC/FIHeld']);
  return {
    date: rocDateToIso(row?.Date) || fallbackDate || null,
    heldShares: shares,
    heldLots: Number.isFinite(shares) ? shares / 1000 : null,
    ratio: number(row?.['PercentageOfSharesOC/FMIHeld']),
    d1Lots: null, d5Lots: null, d10Lots: null, d20Lots: null,
    d1RatioPp: null, d5RatioPp: null, d10RatioPp: null, d20RatioPp: null,
    trendReliable: false,
    structuralChanges: []
  };
}
...
function mergeTpexInstitutionalSnapshot(history, rows) {
  const date = rocDateToIso(rows?.[0]?.Date);
  if (!date || !Array.isArray(rows) || !rows.length) return history;
  const tpexRows = rows.map(row => {
    const value = keys => number(keys.map(key => row?.[key]).find(item => item !== undefined));
    return [String(row?.SecuritiesCompanyCode || '').trim(), {
      foreign: (value(['ForeignInvestorsInclude MainlandAreaInvestors-Difference']) || 0) / 1000,
      trust: (value(['SecuritiesInvestmentTrustCompanies-Difference']) || 0) / 1000,
      dealer: (value(['Dealers-Difference']) || 0) / 1000,
      total: (value(['TotalDifference']) || 0) / 1000
    }];
  }).filter(([code]) => /^\d{4}$/.test(code));
  if (!tpexRows.length) return history;
  const snapshots = history.snapshots.map(snapshot => ({ ...snapshot, rows: new Map(snapshot.rows) }));
  const existingIndex = snapshots.findIndex(snapshot => snapshot.date === date);
  if (existingIndex >= 0) {
    for (const [code, value] of tpexRows) snapshots[existingIndex].rows.set(code, value);
  } else {
    snapshots.unshift({ date, rows: new Map(tpexRows), officialTpex: true });
  }
  snapshots.sort((a, b) => b.date.localeCompare(a.date));
  return { ...history, snapshots: snapshots.slice(0, 20), dates: snapshots.slice(0, 20).map(snapshot => snapshot.date) };
}
...
function tpexCreditFeatures(row, fallbackDate) {
  return {
    date: rocDateToIso(row?.Date) || fallbackDate || null,
    financingBalance: number(row?.MarginPurchaseBalance),
    financingD5: null,
    shortBalance: number(row?.ShortSaleBalance),
    shortD5: null,
    borrowedShortBalance: null,
    borrowedShortD5: null,
    borrowedShortSell5: null,
    financingCrowding: false,
    shortPressure: false
  };
}
...
async function fetchMacroOverlay(twseDailyRows, securitiesRows = []) {
  ...
  const [exportCsv, productionCsv, fx, rate, money] = results.map((r, i) => r.status === 'fulfilled' ? r.value : i < 2 ? '' : {});
  const exportRows = parseCsv(exportCsv).filter(row => Number.isFinite(number(row['統計值(美元)'])));
  exportRows.sort((a, b) => Number(a['資料期(民國年)']) - Number(b['資料期(民國年)']));
  ...
  productionRows.sort((a, b) => Number(a['資料期(民國年)']) - Number(b['資料期(民國年)']));
  ...
  const fxRows = [...(fx.data?.dataSets || [])].sort((a, b) => String(a[0]).localeCompare(String(b[0])));
  const fxLatest = fxRows.at(-1) || [];
  const fxBase = fxRows.at(-21) || [];
  const rateLatest = [...(rate.data?.dataSets || [])].sort((a, b) => String(a[0]).localeCompare(String(b[0]))).at(-1) || [];
  const moneyLatest = [...(money.data?.dataSets || [])].sort((a, b) => String(a[0]).localeCompare(String(b[0]))).at(-1) || [];
  const commonCodes = new Set(securitiesRows.map(row => String(row['公司代號'] || row.Code || '')));
  const commonStocks = (twseDailyRows || []).filter(row => commonCodes.has(String(row.Code || '')));
  ...
  const metrics = {
    ...
    m1bPeriod: /^\d{4}M\d{2}$/.test(moneyLatest[0] || '') ? moneyLatest[0].replace('M', '-') : moneyLatest[0] || null,
  ...
  const monthFresh = value => /^\d{4}-\d{2}$/.test(value || '') && freshness(`${value}-01`, TODAY, 100) === 'current';
  const usable = [Number.isFinite(metrics.exportOrdersYoy) && monthFresh(metrics.exportOrdersPeriod),
    Number.isFinite(metrics.manufacturingYoy) && monthFresh(metrics.manufacturingPeriod),
    Number.isFinite(metrics.m1bYoy) && monthFresh(metrics.m1bPeriod), Number.isFinite(metrics.marketBreadth)];
  let signalScore = 0;
  if (usable[0]) { if (metrics.exportOrdersYoy > 5) signalScore += 1; else if (metrics.exportOrdersYoy < -5) signalScore -= 1; }
...
function stockEtfFeatures(stock, detail, activeSet, activeSignalSet, activeCoverage, laggingSet) {
  ...
  const laggingHolders = (stock.holders || []).filter(holder => laggingSet.has(holder.etf));
  const laggingLots = sum(laggingHolders.map(holder => number(holder.lots) || 0));
  const flowPct = {};
  const activePct = {};
  for (const days of [1, 3, 5, 10, 20]) {
  ...
  return {
    ...
    laggingHolderCount: laggingHolders.length,
    topHolders
  };
}
...
function financialPeriodFromRow(row) {
  ...
  const quarter = row?.['季別'] ?? row?.Quarter;
  if (!year || !quarter) return null;
  const numericYear = Number(year);
  const westernYear = numericYear < 1911 ? numericYear + 1911 : numericYear;
  return `${westernYear}Q${Number(quarter)}`;
}
...
function scoreRecords(records) {
  const fields = ['revenueYoy', 'revenueYtdYoy', 'operatingMargin', 'grossMargin', 'netMargin', 'debtRatio', 'currentRatio', 'pe', 'pb', 'yield',
  ...
  for (const record of records) {
    ...
    const earningsTrendUnit = (
      ...
      0.24 * growthConsistency
    );
    const businessQualityUnit = 0.72 * operatingQuality + 0.28 * profitabilityContinuity;
    const valuationUnit = (
    ...
    const shortOwnershipUnit = (
      0.18 * p(record, 'etfFlow5', true) +
      0.12 * p(record, 'etfFlow10', true) +
      0.12 * p(record, 'activeFlow5', true) +
      0.08 * breadthScore +
      0.14 * p(record, 'foreignNetValue5', true) +
      0.14 * p(record, 'foreignHoldingD5Value', true) +
      0.12 * p(record, 'trustValue5', true) +
      0.06 * trustConsistency +
      0.04 * p(record, 'dealerValue5', true)
    );
    const trendScore = t ? (
      (analysisPrice > t.ema60 ? 0.3 : 0) +
      (analysisPrice > t.ema20 ? 0.2 : 0) +
    ...
    const riskLiquidityUnit = (
      ...
      0.20 * concentrationScore
    );
    const mediumEventRiskUnit = (
      0.45 * eventCatalystUnit +
      0.30 * p(record, 'dailyValue', true) +
      0.25 * volatilityScore
    );
    const fundamentalGuardrailUnit = 0.65 * earningsTrendUnit + 0.35 * businessQualityUnit;

    const shortComponents = {
      technicalTiming: round(30 * technicalUnit, 1),
      ownershipFlow: round(25 * shortOwnershipUnit, 1),
      eventCatalyst: round(15 * eventCatalystUnit, 1),
      riskLiquidity: round(15 * riskLiquidityUnit, 1),
      fundamentalGuardrail: round(15 * fundamentalGuardrailUnit, 1)
    };
    const mediumComponents = {
      earningsTrend: round(25 * earningsTrendUnit, 1),
      businessQuality: round(20 * businessQualityUnit, 1),
      technical: round(20 * technicalUnit, 1),
      valuation: round(15 * valuationUnit, 1),
      ownership: round(10 * ownershipUnit, 1),
      eventRisk: round(10 * mediumEventRiskUnit, 1)
    };
    const longComponents = {
      businessQuality: round(25 * businessQualityUnit, 1),
      financialResilience: round(20 * balanceQuality, 1),
      growthDurability: round(15 * earningsTrendUnit, 1),
      valuation: round(25 * valuationUnit, 1),
    ...
    if (!record.foreignHolding?.trendReliable) {
      comparisonLimits.push('外資持股只有當日快照、缺少5／10日趨勢；此項不給分，也不能解讀為轉弱，跨市場籌碼比較受限');
    }
    const freshnessLimits = [];
    const hasLaggingEtfExposure = record.etf.laggingExposure > 0;
    if (hasLaggingEtfExposure) {
      freshnessLimits.push(`ETF持股中約 ${fmt(record.etf.laggingExposure, 1)}% 來自資料日期非當日的ETF；持股母體仍保留避免漏股，ETF流向不可視為全體ETF同日完整訊號`);
    }
    ...
    record.horizonScores = {
      short: {
        ...
        score: round(sum(Object.values(shortComponents)), 1),
        components: shortComponents
      },
      medium: {
        label: HORIZON_MODELS.medium.label,
        score: round(sum(Object.values(mediumComponents)), 1),
        primary: true,
        components: mediumComponents
      },
      long: {
        label: HORIZON_MODELS.long.label,
    ...
    record.dataHealth = {
      score: round(record.confidence, 1),
      status: healthStatus,
      ...
      missingCore,
      staleCore,
      comparisonStatus: comparisonLimits.length ? '部分可比' : '可比',
      comparisonLimits,
      freshnessLimits,
    ...
    record.rawScore = record.primaryScore;
    record.adjustedScore = record.primaryScore;
...
function classifyRecord(record) {
  ...
  if (!record.dataHealth?.eligible) reasons.push('資料健康度低於65%門檻');
...
function positionDecision(record) {
  ...
  return {
    ...
    recoveryZone: triggerPriceZone(recoveryPrice),
    addZone: triggerPriceZone(t?.ema20)
  };
}
...
async function fetchMisQuotes(records) {
  ...
  for (let i = 0; i < records.length; i += 70) {
    ...
    const exCh = chunk.map(record => `${record.market === 'TPEX' ? 'otc' : 'tse'}_${record.code}.tw`).join('|');
  ...
  for (const payload of payloads) {
    for (const row of payload?.msgArray || []) {
      const match = /_(\w+)\.tw$/i.exec(String(row.c || ''));
      quotes.set(match?.[1] || String(row.c), row);
    }
  }
  return quotes;
}
...
function yahooTechnicalUrl(code, market = 'TWSE') {
  const suffix = market === 'TPEX' ? '.TWO' : '.TW';
  return `https://tw.stock.yahoo.com/quote/${encodeURIComponent(code)}${suffix}/technical-analysis`;
}
...
function recordForOutput(record, rank) {
  return {
    ...
    recoveryZone: record.recoveryZone,
    addZone: record.addZone,
    score: record.adjustedScore,
    rawScore: record.rawScore,
    confidence: record.confidence,
    horizonScores: record.horizonScores,
    ...
    etf: {
      ...
      laggingExposure: round(record.etf.laggingExposure, 2), laggingHolderCount: record.etf.laggingHolderCount, topHolders: record.etf.topHolders
    },
    institution: {
...
function refreshPositionLanguage(report) {
  ...
  for (const row of rows) {
    if (row.holdingState === 'protect') {
      row.holdingAction = '保護持有';
      row.todayAction ||= '今天不新增部位；未完成確認前不機械式賣出';
      row.nextCheck ||= report.meta.quotePhase === 'close' ? '下一交易日收盤，確認是否收復防守區' : '今天收盤後；若跌破，再看下一交易日是否收復';
    } else if (row.holdingState === 'trim') {
      row.holdingAction = '降低部位';
      row.todayAction ||= '停止加碼；反彈未站回恢復區時分批減碼';
      row.nextCheck ||= '每個交易日收盤後，確認是否站回恢復區';
    } else if (row.holdingState === 'hold') {
      row.holdingAction = '正常持有';
      row.todayAction ||= '今天不下單';
      row.nextCheck ||= '每個交易日收盤後，檢查趨勢與法人籌碼';
    } else if (row.holdingState === 'add') {
      row.holdingAction = '符合加碼條件';
...
function buildHtml(report) {
  ...
  const marketScopeNotice = report.meta.unknownMarketStockCount > 0
    ? `<div class="warning warning-scope"><b>ETF 持股範圍提醒：</b>來源快照另含 ${report.meta.unknownMarketStockCount} 筆無法由當日 TWSE／TPEX 官方市場清單辨識的數字代碼（其中可能是海外 ETF 持有的海外股票）；本報告保留在來源稽核計數中，但不納入臺股上市／上櫃排名、不補造價格或評分。</div>`
    : '';
...
<div class="warning warning-news"><b>新聞來源狀態：</b>Yahoo Finance RSS 本次${newsStatusLabel}（成功率 ${fmt(report.meta.yahooNewsCoverageRate, 1)}%）。官方重大訊息與結構化事件已重新檢查，報告照常更新；Yahoo 新聞僅屬待確認資訊，不直接計分。</div>`;
  ...
  const staleEtfNotice = staleEtfs.length
    ? `<div class="warning warning-stale"><b>ETF持股日期覆蓋：</b>${report.meta.etfUpdated}/${report.meta.etfCount} 檔標示為本次日期；${staleEtfs.length} 檔仍為 ${escapeHtml(staleEtfDateSummary)}。為避免漏掉其臺股持有，本報告保留於研究母體；受影響個股會依「ETF資料落後曝險」降低資料健康度，ETF流向不可解讀為全體ETF同日完整訊號。<details><summary>查看 ${staleEtfs.length} 檔日期落後ETF</summary><p>${escapeHtml(staleEtfList)}</p></details></div>`
    : '';
  ...
  const horizonScoreStripHtml = row => `<div class="horizon-score-strip" aria-label="短中長期研究分數"><span><small>短期時機</small><b>${scoreValueLink(row, 'short', row.horizonScores?.short?.score)}</b></span><span class="is-primary"><small>中期研究／排名</small><b>${scoreValueLink(row, 'medium', row.horizonScores?.medium?.score)}</b></span><span><small>長期初篩</small><b>${scoreValueLink(row, 'long', row.horizonScores?.long?.score)}</b><em>方法 ${fmt(row.horizonScores?.long?.methodCoverage, 0)}%／個股資料 ${fmt(row.horizonScores?.long?.dataCoverage, 0)}%</em></span></div>`;
  const topCards = report.topThree.map((row, index) => `
    <article class="pick">
      <div class="pick-head"><span class="rank">${index + 1}</span><div><h3><a class="stock-link" href="${yahooTechnicalUrl(row.code, row.market)}" target="_blank" rel="noreferrer" title="開啟 ${escapeHtml(row.name)} Yahoo技術分析">${escapeHtml(row.code)} ${escapeHtml(row.name)}</a></h3><p>${escapeHtml(row.market === 'TPEX' ? '上櫃' : '上市')}｜${escapeHtml(row.industry)}</p></div><strong class="primary-score"><small>中期排名分</small>${scoreLink(row)}</strong></div>
      ...
      ${horizonScoreStripHtml(row)}
      <div class="metrics"><span>${escapeHtml(report.meta.priceLabel || '最新報價')}<b>${fmt(row.livePrice ?? row.close, 2)}</b></span><span>資料健康<b>${fmt(row.dataHealth?.score, 0)}% ${escapeHtml(row.dataHealth?.status || '')}${row.dataHealth?.comparisonStatus === '部分可比' ? '／部分可比' : ''}</b></span><span>本益比<b>${fmt(row.valuation.pe)}</b></span><span>RSI／K／D<b>${fmt(row.technical.rsi14, 0)}／${fmt(row.technical.kdK, 0)}／${fmt(row.technical.kdD, 0)}</b></span></div>
...
.position-board{border:1px solid #cfd8d3;border-radius:10px;background:#f3f6f4;overflow:hidden;box-shadow:0 18px 42px rgba(18,37,29,.09)}.position-board-head{display:flex;justify-content:space-between;gap:24px;align-items:center;padding:24px 28px;background:#123f30;color:#fff}.position-board-kicker{margin:0 0 6px;color:#d7c17e;font-size:12px;font-weight:900;letter-spacing:.13em}.position-board-head h2{margin:0;font-size:29px}.position-board-head p:last-child{margin:6px 0 0;color:#d9e6df;font-size:14px;line-height:1.55}.position-view-switch{display:grid;grid-template-columns:1fr 1fr;flex:0 0 auto;border:1px solid rgba(255,255,255,.28);border-radius:8px;padding:4px;background:rgba(255,255,255,.08)}.position-view-switch span,.position-view-switch a{min-width:104px;padding:9px 12px;border-radius:5px;color:#fff;font-size:14px;font-weight:800;text-align:center;text-decoration:none}.position-view-switch .is-active{background:#fff;color:#123f30}.position-view-switch a:hover,.position-view-switch a:focus-visible{background:rgba(255,255,255,.14);outline:2px solid #d7c17e;outline-offset:1px}.position-board-body{padding:20px 28px 18px}.position-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.position-summary-card{padding:14px 16px;border:1px solid var(--line);border-top:4px solid var(--blue);border-radius:7px;background:#fff}.position-summary-card.is-trim{border-top-color:#b94b42}.position-summary-card.is-check{border-top-color:#a77517}.position-summary-card.is-hold{border-top-color:#2a6687}.position-summary-card.is-add{border-top-color:#257b56}.position-summary-card b{display:block;font-size:25px;line-height:1}.position-summary-card strong{display:inline-block;margin-top:7px;font-size:14px}.position-summary-card small{margin-left:5px;color:var(--muted);font-size:12px}.position-priority-note{display:flex;gap:12px;align-items:center;margin:12px 0 16px;padding:11px 14px;border:1px solid #dcc994;border-radius:7px;background:#fff8e7;color:#704b0d;font-size:13px;line-height:1.55}.position-priority-note b{flex:0 0 auto;padding:5px 8px;border-radius:3px;background:#86590b;color:#fff;font-size:12px}.position-list-head{display:grid;grid-template-columns:190px 180px minmax(0,1fr) 250px;gap:18px;padding:0 18px 8px;color:#53645c;font-size:12px;font-weight:800}.position-card-list{display:grid;gap:12px}.position-card{display:grid;grid-template-columns:190px 180px minmax(0,1fr) 250px;gap:18px;align-items:stretch;padding:16px 18px;border:1px solid var(--line);border-left:6px solid var(--blue);border-radius:8px;background:#fff}.position-card.tone-trim,.position-card.tone-exit{border-left-color:#b94b42}.position-card.tone-check{border-left-color:#a77517}.position-card.tone-hold{border-left-color:#2a6687}.position-card.tone-add{border-left-color:#257b56}.position-card section{min-width:0}.position-card section+section{padding-left:18px;border-left:1px solid var(--line)}.position-stock-title{display:flex;align-items:baseline;justify-content:space-between;gap:8px}.position-stock-title .stock-link{font-size:17px;font-weight:800}.position-stock-title .score-link{font-size:13px}.position-metrics{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}.position-metrics small,.position-action-pane small,.position-trigger-box small,.position-card-note{display:block;color:var(--muted);font-size:11px}.position-metrics b{display:block;margin-top:3px;font-size:17px}.position-cost{display:flex;align-items:center;gap:7px;margin-top:11px;color:var(--muted);font-size:12px}.position-cost input{width:92px;height:30px;border:1px solid #aebbb4;border-radius:4px;padding:0 7px;background:#fff;font:inherit;color:var(--ink)}.position-card-note{margin:10px 0 0;line-height:1.5}.tracking-toggle{display:flex;align-items:center;gap:5px;margin-top:8px;color:var(--muted);font-size:12px;cursor:pointer}.tracking-toggle input{width:17px;height:17px;accent-color:var(--green)}.position-action-pane{display:flex;flex-direction:column;align-items:flex-start;justify-content:center}.position-action-badge{display:inline-block;padding:7px 12px;border:1px solid currentColor;border-radius:999px;background:#f5f8f6;font-size:15px;font-weight:900}.position-action-badge.action-trim,.position-action-badge.action-exit{color:#aa3d34;background:#fff3f1}.position-action-badge.action-check{color:#8c5d0a;background:#fff8e7}.position-action-badge.action-hold{color:#205d82;background:#f1f7fa}.position-action-badge.action-add{color:#176b49;background:#eff8f3}.position-action-pane small{margin-top:12px}.position-action-pane strong{margin-top:5px;font-size:15px;line-height:1.45}.position-trigger-pane>strong{display:block;font-size:16px;line-height:1.5}.position-trigger-box{display:grid;grid-template-columns:minmax(120px,.55fr) minmax(0,1fr);gap:12px;align-items:center;margin-top:9px;padding:10px 12px;border-radius:6px;background:#f3f6f4}.position-trigger-box b{display:block;font-size:25px;line-height:1.1}.position-trigger-box span{color:var(--muted);font-size:12px;line-height:1.5}.position-trigger-pane p{margin:9px 0 0;color:var(--muted);font-size:12px;line-height:1.55}.position-reasons-pane>strong{font-size:12px;color:#52635b}.position-reason-chips{display:flex;gap:6px;flex-wrap:wrap;margin-top:7px}.position-reason-chips span{padding:5px 7px;border:1px solid #d6dfda;border-radius:4px;background:#f8faf9;color:#4f5f57;font-size:11px;line-height:1.35}.position-reasons-pane details{margin-top:14px}.position-reasons-pane summary{color:#155979;font-size:12px;font-weight:800;cursor:pointer}.position-reasons-pane details p{margin:8px 0 0;color:#4f5f57;font-size:12px;line-height:1.6}.position-empty{padding:24px;border:1px dashed #9fb0a7;border-radius:7px;background:#fff;color:var(--muted);font-size:14px;line-height:1.7}.position-board-footer{display:flex;justify-content:space-between;gap:20px;align-items:flex-end;margin-top:16px;padding-top:14px;border-top:1px solid var(--line)}.position-board-footer .position-actions{margin:0}.position-board-disclaimer{max-width:620px;margin:0;color:var(--muted);font-size:11px;line-height:1.6;text-align:right}
...
@media(max-width:1100px){.position-list-head{display:none}.position-card{grid-template-columns:190px minmax(0,1fr)}.position-card section:nth-child(3){border-left:0;padding-left:0}.position-card .position-trigger-pane,.position-card .position-reasons-pane{padding-top:14px;border-top:1px solid var(--line)}.position-board-footer{align-items:flex-start;flex-direction:column}.position-board-disclaimer{text-align:left}}
@media(max-width:760px){.position-board-head{display:block;padding:21px 18px}.position-board-head h2{font-size:25px}.position-view-switch{margin-top:16px;width:100%}.position-view-switch span,.position-view-switch a{min-width:0}.position-board-body{padding:16px 12px}.position-summary{grid-template-columns:1fr 1fr;gap:8px}.position-summary-card{padding:12px}.position-priority-note{align-items:flex-start}.position-card{grid-template-columns:1fr;gap:14px;padding:15px 14px}.position-card section+section{padding:14px 0 0;border-left:0;border-top:1px solid var(--line)}.position-action-pane{align-items:flex-start}.position-trigger-box{grid-template-columns:1fr}.position-board-footer{gap:10px}.position-board-footer .position-actions{width:100%}.position-action-button{flex:1 1 140px}.position-board-disclaimer{font-size:10px}}
</style>
<style>
...
<style>
 .horizon-mini{display:flex;gap:5px;flex-wrap:wrap;margin-top:10px}.horizon-mini span{padding:4px 6px;border:1px solid var(--line);border-radius:4px;color:var(--muted);font-size:10px}.horizon-mini span.is-primary{border-color:#91b5a1;background:#edf7f1;color:var(--green)}.horizon-mini b{font-size:11px}.position-action-pane>small:first-child{margin:0 0 8px}.position-action-pane .position-action-badge+small{margin-top:13px}.position-trigger-pane>p:first-of-type{margin-top:7px}.score-horizon-note{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0 0 16px}.score-horizon-note div{padding:11px 12px;border:1px solid var(--line);border-radius:5px;background:#f8faf9;color:var(--muted);font-size:12px;line-height:1.55}.score-horizon-note b{display:block;margin-bottom:3px;color:var(--ink)}.score-tabs{display:flex;gap:6px;flex-wrap:wrap;margin:18px 0 12px;border-bottom:1px solid var(--line)}.score-tabs button{border:1px solid var(--line);border-bottom:0;border-radius:5px 5px 0 0;background:#f3f6f4;color:var(--muted);padding:10px 13px;font:inherit;font-weight:800;cursor:pointer}.score-tabs button.is-active,.score-tabs button:focus-visible{background:var(--green);color:#fff;border-color:var(--green);outline:2px solid #c9a85b;outline-offset:2px}.score-tab-intro{display:flex;justify-content:space-between;gap:18px;align-items:center;padding:14px 16px;border:1px solid var(--line);background:#f8faf9}.score-tab-intro div{display:grid;gap:4px}.score-tab-intro span,.score-tab-facts span{color:var(--muted);font-size:12px}.score-tab-intro strong{font-size:30px;color:var(--green);white-space:nowrap}.score-tab-intro strong small{font-size:13px;color:var(--muted)}.score-tab-facts{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 14px}.score-tab-facts span{padding:10px 12px;border:1px solid var(--line);background:#fff;line-height:1.55}.score-tab-facts b{display:block;color:var(--ink);margin-bottom:2px}.score-detail-meta{display:flex;justify-content:space-between;gap:12px;align-items:baseline;margin:14px 0 7px}.score-detail-meta span{color:var(--muted);font-size:12px}.long-coverage-note{display:grid;gap:3px;margin:12px 0;padding:11px 13px;border-left:4px solid var(--amber);background:#fff8e6;line-height:1.55}.long-coverage-note span{font-size:12px;color:var(--muted)}.cross-horizon-reading{margin-top:16px;padding:14px 16px;border:1px solid #b9cdbf;background:#f1f7f4;line-height:1.6}.cross-horizon-reading p{margin:5px 0 0}.score-link{color:var(--blue)}@media(max-width:700px){.score-horizon-note{grid-template-columns:1fr}.score-tab-facts{grid-template-columns:1fr}.score-detail-meta{display:block}.score-detail-meta span{display:block;margin-top:4px}}
 .horizon-mini{display:flex;gap:5px;flex-wrap:wrap;margin-top:10px}.horizon-mini span{padding:4px 6px;border:1px solid var(--line);border-radius:4px;color:var(--muted);font-size:10px}.horizon-mini span.is-primary{border-color:#91b5a1;background:#edf7f1;color:var(--green)}.horizon-mini b{font-size:11px}.position-action-pane>small:first-child{margin:0 0 8px}.position-action-pane .position-action-badge+small{margin-top:13px}.position-trigger-pane>p:first-of-type{margin-top:7px}.score-horizon-note{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0 0 16px}.score-horizon-note div{padding:11px 12px;border:1px solid var(--line);border-radius:5px;background:#f8faf9;color:var(--muted);font-size:12px;line-height:1.55}.score-horizon-note b{display:block;margin-bottom:3px;color:var(--ink)}.score-tabs{display:flex;gap:6px;flex-wrap:wrap;margin:18px 0 12px;border-bottom:1px solid var(--line)}.score-tabs button{border:1px solid var(--line);border-bottom:0;border-radius:5px 5px 0 0;background:#f3f6f4;color:var(--muted);padding:10px 13px;font:inherit;font-weight:800;cursor:pointer}.score-tabs button.is-active,.score-tabs button:focus-visible{background:var(--green);color:#fff;border-color:var(--green);outline:2px solid #c9a85b;outline-offset:2px}.score-tab-intro{display:flex;justify-content:space-between;gap:18px;align-items:center;padding:14px 16px;border:1px solid var(--line);background:#f8faf9}.score-tab-intro div{display:grid;gap:4px}.score-tab-intro span,.score-tab-facts span{color:var(--muted);font-size:12px}.score-tab-intro strong{font-size:30px;color:var(--green);white-space:nowrap}.score-tab-intro strong small{font-size:13px;color:var(--muted)}.score-tab-facts{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 14px}.score-tab-facts span{padding:10px 12px;border:1px solid var(--line);background:#fff;line-height:1.55}.score-tab-facts b{display:block;color:var(--ink);margin-bottom:2px}.score-detail-meta{display:flex;justify-content:space-between;gap:12px;align-items:baseline;margin:14px 0 7px}.score-detail-meta span{color:var(--muted);font-size:12px}.long-coverage-note{display:grid;gap:3px;margin:12px 0;padding:11px 13px;border-left:4px solid var(--amber);background:#fff8e6;line-height:1.55}.long-coverage-note span{font-size:12px;color:var(--muted)}.cross-horizon-reading{margin-top:16px;padding:14px 16px;border:1px solid #b9cdbf;background:#f1f7f4;line-height:1.6}.cross-horizon-reading p{margin:5px 0 0}.score-link{color:var(--blue)}.table-sort-button{display:inline-flex;align-items:center;gap:4px;border:0;background:transparent;color:inherit;padding:2px 3px;font:inherit;font-weight:800;cursor:pointer;white-space:nowrap}.table-sort-button:hover,.table-sort-button:focus-visible{color:var(--green);text-decoration:underline;text-underline-offset:3px;outline:2px solid #c9a85b;outline-offset:2px}.sort-indicator{font-size:12px;color:var(--green)}@media(max-width:700px){.score-horizon-note{grid-template-columns:1fr}.score-tab-facts{grid-template-columns:1fr}.score-detail-meta{display:block}.score-detail-meta span{display:block;margin-top:4px}}
</style>
<style>
/* Keep decision information compact and useful as the browser narrows. */
.position-card{grid-template-columns:190px 190px minmax(0,1fr)}
 .position-stock-title{display:flex;align-items:center;gap:8px;flex-wrap:wrap;cursor:grab;user-select:none}.position-stock-title:active{cursor:grabbing}.position-stock-title a{cursor:pointer;user-select:text}.position-card.is-dragging{opacity:.45}.position-card.is-drag-over{outline:3px solid #c9a85b;outline-offset:2px}.position-reorder{display:inline-flex;gap:4px;margin-left:auto}.position-reorder-button{min-width:34px;height:30px;padding:0 7px;border:1px solid #9eada5;border-radius:4px;background:#fff;color:var(--blue);font:inherit;font-weight:800;cursor:pointer}.position-reorder-button span{font-size:11px}.position-reorder-button:hover,.position-reorder-button:focus-visible{background:#e9f0ec;border-color:var(--green);outline:2px solid #c9a85b;outline-offset:1px}.position-reorder-button:disabled{color:#9aa7a0;border-color:#d8dfdb;background:#f3f6f4;cursor:not-allowed;outline:0}
.position-action-pane small,.position-action-pane strong,.position-trigger-pane>strong,.position-reasons-pane>strong{max-width:100%;overflow-wrap:anywhere}
.position-decision-pane{display:grid;grid-template-columns:minmax(260px,1.15fr) minmax(210px,.85fr);gap:18px;align-items:start}
.position-decision-pane .position-trigger-pane,.position-decision-pane .position-reasons-pane{padding:0;border:0}
.position-decision-pane .position-reasons-pane{min-width:0}
.position-holding-plan{margin:12px 0 0;padding:10px 11px;border-left:3px solid #91b5a1;background:#f3f8f5;color:#405249;font-size:12px;line-height:1.58}
.position-holding-plan b{display:block;margin-bottom:3px;color:var(--ink)}
@media (min-width:761px) and (max-width:1180px){
  .position-card{grid-template-columns:minmax(190px,.8fr) minmax(0,1.5fr);grid-template-areas:"stock action" "decision decision";align-items:start}
  .position-card>section:nth-child(1){grid-area:stock}
  .position-card>section:nth-child(2){grid-area:action}
  .position-card>section:nth-child(3){grid-area:decision;padding:14px 0 0;border-left:0;border-top:1px solid var(--line)}
  .position-card .position-decision-pane{grid-template-columns:minmax(260px,1.15fr) minmax(210px,.85fr)}
}
@media (min-width:761px) and (max-width:900px){
  .position-card .position-decision-pane{grid-template-columns:1fr}
  .position-decision-pane .position-reasons-pane{padding-top:14px;border-top:1px solid var(--line)}
}
@media (max-width:760px){
  .position-card{grid-template-columns:1fr;gap:14px}
  .position-card .position-action-pane,.position-card .position-decision-pane{min-width:0}
  .position-card .position-decision-pane{grid-template-columns:1fr}
  .position-decision-pane .position-reasons-pane{padding-top:14px;border-top:1px solid var(--line)}
...
.score-dates span{padding:6px 9px;border:1px solid #d7e1da;border-radius:7px;background:#f8faf9;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.score-dates span:hover{transform:translateY(-1px);border-color:#8eaf9d;box-shadow:0 5px 12px rgba(18,37,29,.08)}
.score-summary{gap:8px;border:0;background:transparent}.score-summary div{border:1px solid var(--line);border-top:4px solid var(--blue);border-radius:8px;background:#fff;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.score-summary div:nth-child(1){border-top-color:var(--amber)}.score-summary div:nth-child(2){border-top-color:var(--green);background:#f1f8f4}.score-summary div:nth-child(3){border-top-color:var(--blue)}.score-summary div:nth-child(4){border-top-color:var(--gold)}.score-summary div:last-child{border-right:1px solid var(--line)}.score-summary div:hover{transform:translateY(-2px);border-color:#8eaf9d;box-shadow:var(--shadow-hover)}
.score-judgment{border-radius:8px;box-shadow:0 4px 12px rgba(150,103,15,.08);transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.score-judgment:hover{transform:translateY(-2px);box-shadow:var(--shadow-hover);border-left-color:var(--gold)}
.decision-strip{gap:8px;border:0;background:transparent}.decision-strip span{border:1px solid var(--line);border-radius:8px;background:#fff;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.decision-strip span+span{border-left:1px solid var(--line)}.decision-strip span:hover{transform:translateY(-2px);border-color:#8eaf9d;box-shadow:var(--shadow)}
.position-toggle{transition:background .16s ease,border-color .16s ease,box-shadow .16s ease}.position-toggle:hover,.position-toggle:focus-within{background:#eef7f1;border-color:#8eaf9d;box-shadow:var(--shadow)}.position-toggle:focus-within{outline:3px solid var(--gold);outline-offset:2px}
.score-tabs button{transition:transform .16s ease,background .16s ease,border-color .16s ease,color .16s ease,box-shadow .16s ease}.score-tabs button:hover{transform:translateY(-2px);background:#e9f2ec;color:var(--green);border-color:#8eaf9d;box-shadow:0 5px 12px rgba(18,37,29,.1)}.score-tabs button:focus-visible{outline:3px solid var(--gold);outline-offset:2px}
.score-tab-intro,.score-tab-facts span,.long-coverage-note,.cross-horizon-reading{transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.score-tab-intro:hover,.score-tab-facts span:hover,.long-coverage-note:hover,.cross-horizon-reading:hover{transform:translateY(-2px);border-color:#8eaf9d;box-shadow:var(--shadow)}
.score-breakdown tbody tr{transition:background-color .14s ease}.score-breakdown tbody tr:hover td{background:#f1f8f4}.score-breakdown a:focus-visible{outline:3px solid var(--gold);outline-offset:2px;border-radius:3px}
.source-audit{border-radius:12px;box-shadow:var(--shadow);transition:box-shadow .18s ease,border-color .18s ease}.source-audit[open]{border-color:#a8bcb0;box-shadow:var(--shadow-hover)}.source-audit>summary{padding:10px 12px;border-radius:8px;color:var(--green-deep);font-weight:800;transition:background .16s ease,color .16s ease,box-shadow .16s ease}.source-audit>summary:hover{background:#eef7f1;color:var(--green)}.source-audit>summary:focus-visible{outline:3px solid var(--gold);outline-offset:2px;background:#eef7f1}.source-audit>.warning,.source-audit>.quote-phase-banner{border-radius:9px;box-shadow:0 4px 12px rgba(18,37,29,.06);transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease,background .16s ease}.source-audit>.warning:hover,.source-audit>.quote-phase-banner:hover{transform:translateY(-2px);box-shadow:var(--shadow-hover)}.source-audit>.warning{border-left-width:4px}.source-audit>.warning-scope{border-left-color:#6b7d8d;background:#f3f7fa}.source-audit>.warning-news{border-left-color:#7b6a9e;background:#f6f3fb}.source-audit>.warning-stale,.source-audit>.warning-active{border-left-color:var(--amber);background:#fff8e6}.source-audit>.warning-history{border-left-color:var(--blue);background:#f1f7fa}.source-audit>.warning-financial{border-left-color:#8b6b2c;background:#fffaf0}.source-audit>.warning-boundary{border-left-color:var(--green);background:#f1f7f4}.source-audit>.warning details summary{display:inline-block;margin-top:7px;padding:4px 7px;border-radius:5px;color:var(--green-deep);font-weight:800;transition:background .16s ease,box-shadow .16s ease}.source-audit>.warning details summary:hover{background:#e8f2ea;box-shadow:0 3px 8px rgba(18,37,29,.1)}.source-audit>.warning details summary:focus-visible{outline:3px solid var(--gold);outline-offset:2px}.source-audit>.warning details p{margin:8px 0 0;color:var(--muted);line-height:1.6}
.summary-grid div,.decision-legend div,.position-summary-card,.position-priority-note,.framework-note{transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease,background .16s ease}.summary-grid div:hover,.decision-legend div:hover,.position-summary-card:hover,.position-priority-note:hover,.framework-note:hover{transform:translateY(-2px);border-color:#8eaf9d;box-shadow:var(--shadow-hover)}.summary-grid div{border-radius:8px}.decision-legend div{border-radius:8px}.position-summary-card{border-radius:8px}.position-priority-note{border-radius:8px}
.filters input,.filters select{transition:border-color .16s ease,box-shadow .16s ease,background .16s ease}.filters input:hover,.filters select:hover{border-color:#8eaf9d;box-shadow:0 3px 9px rgba(18,37,29,.08)}.filters input:focus-visible,.filters select:focus-visible{outline:3px solid var(--gold);outline-offset:2px;border-color:var(--green)}
.horizon-score-strip span,.score-horizon-note div,.position-trigger-box,.position-reason-chips span,.quick-action{transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease,background-color .16s ease}.horizon-score-strip span:hover,.score-horizon-note div:hover,.position-trigger-box:hover,.position-reason-chips span:hover,.quick-action:hover{transform:translateY(-2px);border-color:#8eaf9d;box-shadow:var(--shadow)}.horizon-score-strip span:hover,.score-horizon-note div:hover{background:#f1f8f4}.position-trigger-box:hover{background:#eef7f1}.position-reason-chips span:hover{background:#eef7f1;color:var(--green-deep)}.quick-action:hover{background:#e1f1e7}
@media(prefers-reduced-motion:reduce){*,*::before,*::after{scroll-behavior:auto!important;transition-duration:.01ms!important;animation-duration:.01ms!important;animation-iteration-count:1!important}}
button:focus-visible,input:focus-visible,select:focus-visible,summary:focus-visible,a:focus-visible{outline:3px solid var(--gold);outline-offset:2px}
...
${marketScopeNotice}
${quotePhaseNotice}
<div class="warning warning-history"><b>外資持股歷史完整性：</b>本次由證交所取得 ${report.meta.foreignHoldingHistoryDays} 個有效交易日；至少 11 日才計算並發布 10 日持股變化。</div>
<div class="warning warning-active"><b>主動ETF當日完整性：</b>${report.meta.activeUpdated}/${report.meta.activeEtfs} 檔（${fmt(report.meta.activeCoverageRate, 1)}%）。${report.meta.activeEtfDataComplete ? '完整，可使用主動ETF訊號判定新建部位。' : `部分來源尚未完成核對：${escapeHtml(report.meta.activeStaleEtfs.map(etf => `${etf.code} ${etf.name}`).join('、'))}；新部位只在個股完整門檻與承接區同時成立時執行。`}</div>
${staleEtfNotice}
<div class="warning warning-financial" id="financialCoverageBanner"><b>季報申報過渡期：</b>官方最新申報季為 ${escapeHtml(report.meta.financialCurrentPeriod || '未確認')}；本次 ${report.meta.financialCurrentCount} 檔使用本次官方端點資料、${report.meta.financialFallbackCount} 檔明確沿用先前已驗證官方快照（同季或前一季）、${report.meta.financialUnavailableCount} 檔改由個股其他可驗證條件判讀。個股評分明細會標示實際季別與來源狀態，不把歷史資料冒充本次新取得資料。</div>
<div class="warning warning-boundary"><b>資料邊界：</b>研究母體是 ${report.meta.etfCount} 檔 ETF 所持有且可辨識的 ${report.meta.stockCount} 檔上市／上櫃普通股，已涵蓋本次 ETF 持股資料中的 ${report.meta.allEtfHeldStocks} 檔可辨識四碼普通股（上市 ${report.meta.twseHeldStockCount} 檔／上櫃 ${report.meta.tpexHeldStockCount} 檔）。ETF持股母體來自B級籌碼小宇快照（${escapeHtml(report.meta.etfSourceGeneratedAt || '未提供')}），證交所／櫃買中心官方清單只確認市場別，尚未逐檔與投信官方持股清單對帳。主動ETF是否可用於新建部位，僅以上方「主動ETF當日完整性」的明確 ${report.meta.activeUpdated}/${report.meta.activeEtfs} 門檻判定；其餘ETF的實際持股日期與落後曝險見上方日期覆蓋說明。上櫃股票的上市專屬官方欄位若無資料會保留缺漏，不以中性值補造。ETF 20日只作背景、10日看延續、5日看轉折。法人買賣超最新窗來源為 ${escapeHtml(report.meta.institutionalSource)}，官方未滿20日時才以B級歷史補齊；外資持股存量仍與買賣超流量分開。宏觀、信用交易與集保是獨立覆蓋，不重複灌入100分。評分是研究優先排序，不是保證報酬或個人化投資建議。</div>

</details>
<details class="source-audit"><summary>臺灣景氣與貨幣背景</summary><section class="section"><h2>宏觀與市場環境覆蓋</h2><p class="section-lead">狀態：<b>${escapeHtml(report.macroOverlay.status)}</b>（訊號分數 ${fmt(report.macroOverlay.signalScore, 0)}）。本層使用官方公開資料，只調整研究時的環境認知，不直接改個股100分與排名。</p><div class="summary-grid"><div><b>${signed(report.macroOverlay.metrics.exportOrdersYoy, 1, '%')}</b><span>外銷訂單年增｜${escapeHtml(report.macroOverlay.metrics.exportOrdersPeriod)}</span></div><div><b>${signed(report.macroOverlay.metrics.manufacturingYoy, 1, '%')}</b><span>製造業生產年增｜${escapeHtml(report.macroOverlay.metrics.manufacturingPeriod)}</span></div><div><b>${signed(report.macroOverlay.metrics.m1bYoy, 1, '%')}</b><span>M1B年增｜${escapeHtml(report.macroOverlay.metrics.m1bPeriod)}</span></div><div><b>${fmt(report.macroOverlay.metrics.marketBreadth, 1)}%</b><span>上市普通股上漲家數占比｜${report.macroOverlay.metrics.advances}漲／${report.macroOverlay.metrics.declines}跌</span></div></div><p class="framework-note">央行歷史匯率 ${fmt(report.macroOverlay.metrics.usdTwd, 3)}（${escapeHtml(report.macroOverlay.metrics.usdTwdDate)}；近20日變化 ${signed(report.macroOverlay.metrics.usdTwdChange20, 2, '%')}）；匯率時效 ${escapeHtml(report.macroOverlay.fxStatus || '未確認')}。最新期交所參考值見上方匯市卡。重貼現率 ${fmt(report.macroOverlay.metrics.policyRate, 3)}%（生效日 ${escapeHtml(report.macroOverlay.metrics.policyRateDate)}）。</p></section>

</details>
<details class="source-audit"><summary>前三名研究候選與完整說明</summary><section class="section"><h2>本次執行的前三名研究候選</h2><p class="section-lead">這是依「中期研究分數＋硬門檻」重算後的研究優先順序，不是持倉清單，也不代表掉出名單就該賣出。短期看時機、長期看品質初篩；已開始布局者請勾選追蹤。</p><div class="picks">${topCards}</div><div class="decision-legend"><div><b>尚未持有</b><p>只有「可開始承接」才進入分批布局；等待確認、暫不承接或不建立部位都不應因排名高而追價。</p></div><div><b>已經持有</b><p>先看正常／保護持有、符合加碼條件、降低部位或優先降低風險，再看今天動作與下一次確認；排名變化本身不是賣出條件。</p></div></div></section>

</details>
<section class="section position-board" id="positionSection" aria-labelledby="positionBoardTitle">
   <div class="position-board-head">
     <div><p class="position-board-kicker">PORTFOLIO ACTION BOARD</p><h2 id="positionBoardTitle">持股決策總覽</h2><p>依序看：目前狀態、今天動作、下一次確認、改變條件。中期排名不是單獨買賣理由。可拖曳左側股票區塊調整順序。</p></div>
    <nav class="position-view-switch" aria-label="持股決策檢視"><span class="is-active" aria-current="page">我已持有</span><a href="#fullRankingSection">我尚未持有</a></nav>
  </div>
  <div class="position-board-body">
    <div class="position-summary" id="positionDecisionSummary" aria-label="持股動作統計">
      <div class="position-summary-card is-trim"><b id="positionSummaryTrimCount">0</b><strong>減碼／出脫</strong><small id="positionSummaryTrimCodes">目前無標的</small></div>
      <div class="position-summary-card is-check"><b id="positionSummaryCheckCount">0</b><strong>等待確認</strong><small id="positionSummaryCheckCodes">目前無標的</small></div>
      <div class="position-summary-card is-hold"><b id="positionSummaryHoldCount">0</b><strong>正常／保護持有</strong><small id="positionSummaryHoldCodes">目前無標的</small></div>
      <div class="position-summary-card is-add"><b id="positionSummaryAddCount">0</b><strong>符合加碼條件</strong><small id="positionSummaryAddCodes">目前無標的</small></div>
    </div>
    <div class="position-priority-note"><b>最高優先</b><span>若出現重大風險、基本面破壞或趨勢惡化，直接覆蓋加碼訊號；先處理減碼與出脫，再看其他標的。</span></div>
    <div class="position-list-head" aria-hidden="true"><span>股票與現況</span><span>目前狀態／今天動作</span><span>下一步與條件</span><span>主要原因</span></div>
    <div class="position-card-list" id="positionRows"></div>
    <div class="position-board-footer">
      <div class="position-actions"><button class="position-action-button" id="exportPositions" type="button" title="下載布局追蹤JSON備份">&#8595; 下載追蹤備份</button><button class="position-action-button" id="importPositions" type="button" title="從JSON備份匯入布局追蹤">&#8593; 匯入追蹤備份</button><input id="importPositionsFile" type="file" accept="application/json,.json" hidden><span class="position-backup-status" id="positionBackupStatus" role="status" aria-live="polite"></span></div>
      <p class="position-board-disclaimer"><b>閱讀順序：</b>先看「目前狀態」與「今天動作」，再看「下一次確認」與「改變條件」。價位以交易跳動單位呈現觀察區，不是保證成交或自動停損指令。</p>
    </div>
  </div>
</section>

<section class="section"><h2>篩選覆蓋</h2><div class="summary-grid"><div><b>${report.meta.stockCount}／${report.meta.allEtfHeldStocks}</b><span>本次納入普通股／ETF持股可辨識母體</span></div><div><b>${report.meta.foreignHoldingCovered}</b><span>具官方外資持股趨勢</span></div><div><b>${report.meta.bucketA}</b><span>A級：通過初步布局門檻</span></div><div><b>${report.meta.rejected}</b><span>D級：硬性條件排除</span></div></div></section>

  <section class="section"><h2>前30名與主要風險</h2><p class="section-lead">排名只使用「中期研究分數」；短期分數回答時機，長期分數是方法覆蓋85%的初篩，資料健康度則獨立顯示。點擊分數可查看明細；點選 #、短期、中期或長期表頭可切換由小到大／由大到小排序。</p><div class="table-scroll-tools"><button class="scroll-button" id="top30ScrollLeft" type="button" title="向左捲動" aria-label="前30名表格向左捲動">&#8592;</button><div class="top-scroll" id="top30TopScroll" aria-label="前30名表格水平捲軸"><div class="top-scroll-sizer" id="top30TopScrollSizer"></div></div><button class="scroll-button" id="top30ScrollRight" type="button" title="向右捲動" aria-label="前30名表格向右捲動">&#8594;</button></div><div class="table-wrap top30-table-wrap" id="top30TableWrap"><table class="top30-table" id="top30Table"><thead><tr><th>${sortButton('#','rank')}</th><th>股票</th><th>追蹤</th><th>建立新部位</th><th>已經持有</th><th>${sortButton('短期時機','short')}</th><th>${sortButton('中期研究／排名','medium')}</th><th>${sortButton('長期初篩','long')}</th><th>資料健康</th><th>營收YoY</th><th>PE</th><th>ETF 5日</th><th>主動ETF 5日</th><th>外資持股5日</th><th>外資持股10日</th><th>外資買賣超5日</th><th>投信買賣超5日</th><th>RSI</th><th>K／D</th><th>最先注意風險</th></tr></thead><tbody>${topRows}</tbody></table></div></section>

 <section class="section" id="fullRankingSection"><h2>完整 ${report.meta.stockCount} 檔上市股票排名</h2><div class="filters"><input id="search" type="search" placeholder="搜尋代號或名稱"><select id="bucket"><option value="">全部新部位動作</option><option value="A">可開始承接</option><option value="B">等待確認</option><option value="C">暫不承接</option><option value="D">不建立部位</option></select><select id="sector"><option value="">全部產業</option>${[...new Set(report.ranking.map(row => row.sector))].sort().map(value => `<option>${escapeHtml(value)}</option>`).join('')}</select><label class="position-toggle"><input id="positionOnly" type="checkbox"> <span>只看我的布局追蹤</span></label></div><div class="table-scroll-tools"><button class="scroll-button" id="scrollLeft" type="button" title="向左捲動" aria-label="向左捲動">&#8592;</button><div class="top-scroll" id="fullTopScroll" aria-label="表格水平捲軸"><div class="top-scroll-sizer" id="fullTopScrollSizer"></div></div><button class="scroll-button" id="scrollRight" type="button" title="向右捲動" aria-label="向右捲動">&#8594;</button></div><div class="table-wrap full-table-wrap" id="fullTableWrap"><table class="full-table" id="fullTable"><thead><tr><th>${sortButton('#','rank')}</th><th>股票</th><th>追蹤</th><th>產業</th><th>建立新部位</th><th>已經持有</th><th>${sortButton('短期時機','short')}</th><th>${sortButton('中期研究／排名','medium')}</th><th>${sortButton('長期初篩','long')}</th><th>資料健康</th><th>即時/收盤</th><th>營收YoY</th><th>累計YoY</th><th>PE</th><th>ETF 5日</th><th>ETF 10日</th><th>主動5日</th><th>外資持股比</th><th>外資持股5日</th><th>外資持股10日</th><th>外資買賣超5日</th><th>投信5日</th><th>投信10日</th><th>RSI</th><th>K／D</th><th>距20EMA</th><th>主要風險</th></tr></thead><tbody id="fullRows"></tbody></table></div><p class="small">中期研究分數決定排名；短期與長期分數提供不同時間尺度，不可互相替代。點選 # 或三個時間尺度表頭可排序顯示，但不改變研究排名、分數或投資動作。外資持股比率高低本身不是買賣訊號，重點是持股存量、法人買賣流量與ETF方向是否一致。</p></section>

<section class="section"><h2>三時間尺度評分架構</h2><p class="section-lead">三個分數回答不同問題，不能互相替代；畫面以整數顯示，底層保留一位小數供稽核。只有中期研究分數用於排名，資料健康度不加分、不扣分，只作獨立品質門檻。</p><div class="framework-wrap"><table class="framework-table"><thead><tr><th>用途</th><th>實際權重</th><th>回答的問題</th><th>限制與使用方式</th></tr></thead><tbody>
<tr><td><b>短期時機（100分）</b></td><td><ul><li>技術與時機 30%</li><li>ETF／法人短期流向 25%</li><li>事件催化 15%</li><li>風險與流動性 15%</li><li>基本面護欄 15%</li></ul></td><td>現在是不是相對合理的觀察、等待或分批時機？</td><td>不預測單日漲跌；盤中訊號一律等收盤確認。事件項不再重複放入營收成長。</td></tr>
<tr><td><b>中期研究（100分，排名主軸）</b></td><td><ul><li>盈餘與營收趨勢 25%</li><li>企業營運品質 20%</li><li>技術趨勢 20%</li><li>估值 15%</li><li>ETF／法人籌碼 10%</li><li>事件、風險與流動性 10%</li></ul></td><td>未來數週至數月，哪一些股票最值得優先深入研究？</td><td>本益比只在估值計分；營收只在趨勢構面計分；仍須先通過資料、流動性、趨勢與法人等硬門檻。</td></tr>
<tr><td><b>長期初篩（正規化100分）</b></td><td><ul><li>企業營運品質 25%</li><li>財務韌性 20%</li><li>成長耐久性 15%</li><li>估值 25%</li><li>資本配置品質 15%：尚未計分</li></ul></td><td>哪些公司具備較值得長期研究的品質與價格條件？</td><td>目前方法覆蓋85%，再正規化為100分以便比較；因尚缺完整自由現金流、ROIC與資本配置紀律，只能稱「初篩」，不可單獨形成長期買賣動作。</td></tr>
<tr><td><b>資料健康度（獨立）</b></td><td>ETF歷史、月營收、季報、估值、技術、法人、外資持股及資料落後程度</td><td>這筆研究是否有足夠資料可採用？</td><td>不進入任何投資分數，也不乘上分數係數；低於65%不得進入A級新部位候選。</td></tr>
</tbody></table></div><p class="framework-note">反重複計分：本益比不再與盈餘殖利率重複；營收不再同時灌入基本面與催化；資料健康度不再出現在風險分數、總分乘數與硬門檻三個位置。高分若觸發硬性風險，仍會降級或排除。</p></section>
...
<section class="section"><h2>證據層級與限制</h2><p>A級：官方原始資料｜B級：次級行情與ETF快照｜C級：待確認新聞。來源可信度不等於策略勝率。</p><details><summary>展開完整依據與資料缺口</summary><p class="section-lead">A級證據優先採證交所、櫃買中心、集保結算所、經濟部及中央銀行的官方公開資料。B級次級資料包含籌碼小宇的ETF持股／歷史價格，以及Yahoo Finance日K高低收；前者用於ETF母體與持股變化，後者只作技術與標準KD輔助，兩者都不是官方ETF逐檔持股對帳，不能稱為官方持股驗證。ETF持股日期以各基金來源明細為準；日期落後ETF仍保留於母體避免漏股，但其曝險會降低個股資料健康度，ETF流向不可視為全體基金同日完整訊號。Yahoo Finance RSS新聞是C級待確認資訊，不直接計分。官方重大訊息已接入；法說會只在重大訊息文字明確出現「法人說明會／法說會」時辨識，不宣稱已取得完整法說會資料庫。結構化庫藏股與處置事件只進入各時間尺度的單一事件構面，官方重大訊息、信用交易與集保作獨立風險覆蓋，避免重複計分。外資持股變化仍可能受借券、海外存託憑證、股本異動與ETF申購買回等因素影響。本報告尚未取得券商一致預估、目標價、完整自由現金流、ROIC、資本配置紀律與個人風險承受度。</p><ul class="sources">${sourceRows}</ul></details></section>
</main>
<dialog class="score-dialog" id="scoreDialog" aria-labelledby="scoreDialogTitle"><div class="score-dialog-shell"><header class="score-dialog-head"><div><h2 id="scoreDialogTitle">股票評分明細</h2><p id="scoreDialogSub">短／中／長期分數、資料健康度、判斷數據與來源</p></div><button class="score-close" id="scoreClose" type="button" title="關閉評分明細" aria-label="關閉評分明細">&times;</button></header><div class="score-dialog-body" id="scoreDialogBody"></div></div></dialog>
</div>
<script>
...
const AUTH_ACCOUNTS=[{username:'frica',password:'stock2026'},{username:'Amanda',password:'frica'},{username:'frank',password:'stock2026'}];const AUTH_STORAGE_KEY='pro-ranking-auth-v1';
...
function stockUrl(code,market){const row=rows.find(item=>item.code===String(code));const suffix=(market||row?.market)==='TPEX'?'.TWO':'.TW';return 'https://tw.stock.yahoo.com/quote/'+encodeURIComponent(code)+suffix+'/technical-analysis'}
 function scoreAnchor(r,tab='medium',value=r.score){return '<a class="score-link" href="#score-'+encodeURIComponent(r.code)+'" data-score-code="'+e(r.code)+'" data-score-tab="'+tab+'" title="查看 '+e(r.name)+' 的'+(tab==='short'?'短期':tab==='long'?'長期':'中期')+'評分明細">'+n(value,0)+'</a>'}
 function horizonMini(r){const h=r.horizonScores||{};return '<div class="horizon-mini" aria-label="短中長期研究分數"><span>短 '+scoreAnchor(r,'short',h.short&&h.short.score)+'</span><span class="is-primary">中 '+scoreAnchor(r,'medium',h.medium&&h.medium.score)+'</span><span>長 '+scoreAnchor(r,'long',h.long&&h.long.score)+'</span></div>'}
 const tableSortState={top30:null,full:null};
 const sortColumnIndex={top30:{rank:0,short:5,medium:6,long:7},full:{rank:0,short:6,medium:7,long:8}};
 function updateSortButtons(tableName){const state=tableSortState[tableName];document.querySelectorAll('[data-table-sort]').forEach(button=>{const table=button.closest('table');if(!table||table.id!==(tableName==='top30'?'top30Table':'fullTable'))return;const active=state&&state.key===button.dataset.tableSort;button.closest('th').setAttribute('aria-sort',active?(state.direction==='asc'?'ascending':'descending'):'none');button.querySelector('.sort-indicator').textContent=active?(state.direction==='asc'?'↑':'↓'):'↕';});}
 function scoreNumber(value){const parsed=Number(String(value).replace(/,/g,''));return Number.isFinite(parsed)?parsed:-Infinity}
 function sortTop30Rows(){const state=tableSortState.top30;if(!state)return;const tbody=top30Table.tBodies[0];const index=sortColumnIndex.top30[state.key];const rows=[...tbody.rows];rows.sort((a,b)=>{const diff=scoreNumber(a.cells[index].innerText)-scoreNumber(b.cells[index].innerText);return (state.direction==='asc'?diff:-diff)||Number(a.cells[0].innerText)-Number(b.cells[0].innerText)});rows.forEach(row=>tbody.appendChild(row));updateSortButtons('top30');requestAnimationFrame(syncTop30ScrollWidth)}
 function setTableSort(key){const tableName=top30Table.contains(document.activeElement)?'top30':'full';const current=tableSortState[tableName];tableSortState[tableName]=current&&current.key===key?{key,direction:current.direction==='asc'?'desc':'asc'}:{key,direction:'asc'};if(tableName==='top30')sortTop30Rows();else draw();updateSortButtons(tableName)}
...
function sourceLinks(key,code){return '<div class="source-links">'+(sourceGroups[key]||[]).map(item=>{let url=item.url;if(key==='technical'&&url.endsWith('/data/stock/'))url+=encodeURIComponent(code)+'.json';url=url.replace('{code}',encodeURIComponent(code));return '<a href="'+e(url)+'" target="_blank" rel="noreferrer">'+e(item.label)+'</a>'}).join('')+'</div>'}
  function componentRow(label,score,max,evidence,key,code){const points=score===null?'尚未計分':n(score);return '<tr><td>'+e(label)+'</td><td>'+points+' / '+n(max,0)+'</td><td>'+evidenceList(evidence)+'</td><td>'+sourceLinks(key,code)+'</td></tr>'}
  let scoreTabContent={};
  function scoreTab(tab){const item=scoreTabContent[tab];if(!item)return;document.querySelectorAll('[data-score-tab]').forEach(button=>{const selected=button.dataset.scoreTab===tab;button.classList.toggle('is-active',selected);button.setAttribute('aria-selected',selected?'true':'false');});const panel=document.getElementById('scoreTabPanel');if(panel)panel.innerHTML=item}
  function crossHorizonReading(h){const short=Number(h.short&&h.short.score);const medium=Number(h.medium&&h.medium.score);const long=Number(h.long&&h.long.score);const strong=value=>Number.isFinite(value)&&value>=65;const weak=value=>Number.isFinite(value)&&value<45;if(strong(short)&&weak(medium)&&weak(long))return '長期結論：不列入長期優先；短期偏強只可做短線觀察，不等於長期買進。';if(weak(short)&&strong(medium)&&strong(long))return '長期結論：可列入長期研究；今天不買，等短期轉強且個股卡顯示可開始承接。';if(strong(short)&&strong(medium)&&weak(long))return '長期結論：不適合長期持有；只作短中期波段研究。';if(weak(short)&&weak(medium)&&strong(long))return '長期結論：可列長期觀察，但目前不買；等個股卡進入可開始承接。';return '長期結論：方向未完全一致；未顯示可開始承接就不買，已持有則依個股卡維持或減碼。'}
...
function currentPrice(r){return Number.isFinite(r.livePrice)?r.livePrice:r.close}
 function nextPositionOrder(){return Object.values(positions).reduce((max,item)=>Math.max(max,Number.isFinite(Number(item.positionOrder))?Number(item.positionOrder):-1),-1)+1}
 function ensurePositionOrder(){let changed=false;let next=nextPositionOrder();Object.values(positions).forEach(item=>{if(!Number.isFinite(Number(item.positionOrder))){item.positionOrder=next++;changed=true}});if(changed)savePositions()}
 function setPosition(code,enabled){const r=rows.find(row=>row.code===String(code));if(!r)return;if(enabled&&!positions[r.code])positions[r.code]={code:r.code,addedAt:reportMeta.generatedAt,entryPrice:currentPrice(r),entryRank:r.rank,entryAction:r.entryAction,positionOrder:nextPositionOrder()};if(!enabled)delete positions[r.code];savePositions();renderPositions();draw();syncPositionChecks()}
...
function positionDecisionMeta(p,r){
  ...
  else if(r.holdingState==='protect'){key='hold';label='保護持有';when='尚未觸發減碼';todayAction='今天不新增部位';nextCheck=intraday?'今天收盤後':'下一交易日收盤後';trigger=ema60;zone=r.defenseZone;triggerLabel='防守觀察區';instruction='目前尚未完成減碼確認，維持部位但停止加碼。';change='收盤跌破觀察區且下一交易日未收復 → 先減碼1/3'}
  const priceGap=Number.isFinite(trigger)&&trigger>0?(price-trigger)/trigger*100:null;const costGap=Number.isFinite(trigger)&&trigger>0&&Number.isFinite(cost)&&cost>0?(trigger-cost)/cost*100:null;
  const gapText=Number.isFinite(priceGap)?(priceGap>=0?'現價高於門檻 '+n(priceGap,1)+'%':'現價低於門檻 '+n(Math.abs(priceGap),1)+'%'):'缺少可驗證價位';
  const costText=Number.isFinite(costGap)?'；門檻較成本 '+(costGap>=0?'+':'')+n(costGap,1)+'%':'';
  const zoneLow=Number(zone&&zone.low);const zoneHigh=Number(zone&&zone.high);const digits=Number.isFinite(trigger)&&trigger<50?2:Number.isFinite(trigger)&&trigger<500?1:0;const zoneText=Number.isFinite(zoneLow)&&Number.isFinite(zoneHigh)?n(zoneLow,digits)+'–'+n(zoneHigh,digits):(Number.isFinite(trigger)&&trigger>0?n(trigger,digits):'—');
...
function renderPositionSummary(items){
  ...
  [['Trim','trim'],['Check','check'],['Hold','hold'],['Add','add']].forEach(pair=>{const label=pair[0],key=pair[1],codes=groups[key];document.getElementById('positionSummary'+label+'Count').textContent=String(codes.length);document.getElementById('positionSummary'+label+'Codes').textContent=codes.length?codes.join('、'):'目前無標的'})
}
 function movePosition(code,direction){const ordered=Object.values(positions).sort((a,b)=>Number(a.positionOrder)-Number(b.positionOrder));const index=ordered.findIndex(item=>item.code===String(code));const target=index+direction;if(index<0||target<0||target>=ordered.length)return;const current=ordered[index].positionOrder;ordered[index].positionOrder=ordered[target].positionOrder;ordered[target].positionOrder=current;savePositions();renderPositions()}
 function movePositionTo(code,targetIndex){const ordered=Object.values(positions).sort((a,b)=>Number(a.positionOrder)-Number(b.positionOrder));const sourceIndex=ordered.findIndex(item=>item.code===String(code));if(sourceIndex<0||targetIndex<0||targetIndex>=ordered.length||sourceIndex===targetIndex)return;const [item]=ordered.splice(sourceIndex,1);ordered.splice(targetIndex,0,item);ordered.forEach((entry,index)=>{entry.positionOrder=index});savePositions();renderPositions()}
 let draggedPositionCode='';
 let pointerDragMoved=false;
 function clearDragState(){document.querySelectorAll('.position-card.is-dragging,.position-card.is-drag-over').forEach(card=>card.classList.remove('is-dragging','is-drag-over'))}
 function positionDropIndexAt(target,clientY){const cards=[...positionRows.querySelectorAll('.position-card')];if(!target)return null;const sourceIndex=cards.findIndex(card=>card.dataset.positionCode===draggedPositionCode);const targetIndex=cards.indexOf(target);if(sourceIndex<0||targetIndex<0)return null;const after=clientY>target.getBoundingClientRect().top+target.getBoundingClientRect().height/2;let index=after?targetIndex+1:targetIndex;if(sourceIndex<index)index--;return Math.max(0,Math.min(cards.length-1,index))}
 function positionDropIndex(event){return positionDropIndexAt(event.target.closest('.position-card'),event.clientY)}
  positionRows.addEventListener('pointerdown',event=>{const handle=event.target.closest('[data-position-drag-handle]');if(!handle||event.button!==0||event.target.closest('button,input,label,a'))return;draggedPositionCode=handle.dataset.positionDragHandle;pointerDragMoved=false;handle.setPointerCapture?.(event.pointerId);handle.closest('.position-card').classList.add('is-dragging')});
 positionRows.addEventListener('pointermove',event=>{if(!draggedPositionCode)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest('.position-card');if(!target)return;if(Math.abs(event.movementX)+Math.abs(event.movementY)>3){pointerDragMoved=true;event.preventDefault()}positionRows.querySelectorAll('.position-card.is-drag-over').forEach(item=>item.classList.remove('is-drag-over'));if(target.dataset.positionCode!==draggedPositionCode)target.classList.add('is-drag-over')});
 positionRows.addEventListener('pointerup',event=>{if(!draggedPositionCode)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest('.position-card');const code=draggedPositionCode;const targetIndex=positionDropIndexAt(target,event.clientY);const moved=pointerDragMoved;clearDragState();draggedPositionCode='';pointerDragMoved=false;if(moved){event.preventDefault();if(targetIndex!==null)movePositionTo(code,targetIndex)}});
 positionRows.addEventListener('pointercancel',()=>{if(draggedPositionCode){clearDragState();draggedPositionCode='';pointerDragMoved=false}});
  document.addEventListener('dragstart',event=>{const handle=event.target.closest('[data-position-drag-handle]');if(!handle||event.target.closest('button,input,label,a')){if(handle)event.preventDefault();return}const card=handle.closest('.position-card');if(!card)return;draggedPositionCode=card.dataset.positionCode;card.classList.add('is-dragging');event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',draggedPositionCode)});
 positionRows.addEventListener('dragover',event=>{if(!draggedPositionCode)return;const card=event.target.closest('.position-card');if(!card)return;event.preventDefault();event.dataTransfer.dropEffect='move';positionRows.querySelectorAll('.position-card.is-drag-over').forEach(item=>item.classList.remove('is-drag-over'));if(card.dataset.positionCode!==draggedPositionCode)card.classList.add('is-drag-over')});
 positionRows.addEventListener('drop',event=>{if(!draggedPositionCode)return;event.preventDefault();const targetIndex=positionDropIndex(event);const code=draggedPositionCode;clearDragState();draggedPositionCode='';if(targetIndex!==null)movePositionTo(code,targetIndex)});
 document.addEventListener('dragend',event=>{if(event.target.closest('[data-position-drag-handle]')){clearDragState();draggedPositionCode=''}});
 function renderPositions(){
   ensurePositionOrder();
   const tracked=Object.values(positions).map(p=>({p,r:rows.find(row=>row.code===String(p.code))})).sort((a,b)=>Number(a.p.positionOrder)-Number(b.p.positionOrder));
  if(!tracked.length){renderPositionSummary([]);positionRows.innerHTML='<div class="position-empty"><b>尚未標記任何布局部位。</b><br>請在前三名、前30名或完整排名中勾選「追蹤」；系統會保存成本價與起始排名，之後套用最新判斷。</div>';return}
  const items=tracked.map(item=>({...item,view:item.r?positionDecisionMeta(item.p,item.r):null}));
  renderPositionSummary(items);
  positionRows.innerHTML=items.map(({p,r,view})=>{
    if(!r)return '<div class="position-empty"><b>'+e(p.code)+'</b> 目前不在本次ETF持股上市股票母體，需另行查核。</div>';
     const price=currentPrice(r);const cost=Number(p.entryPrice);const pnl=Number.isFinite(cost)&&cost>0?(price-cost)/cost*100:null;const rankMove=Number.isFinite(Number(p.entryRank))?Number(p.entryRank)-r.rank:null;const rankText=e(p.entryRank)+' → '+e(r.rank)+(rankMove===null?'':rankMove>0?'（上升 '+n(rankMove,0)+'）':rankMove<0?'（下降 '+n(Math.abs(rankMove),0)+'）':'（不變）');const reasons=decisionReasonChips(r);const basis=reasons[0]||'目前未觸發明確減碼訊號';
     const positionIndex=items.findIndex(item=>item.p.code===r.code);const moveButtons='<div class="position-reorder" role="group" aria-label="調整 '+e(r.name)+' 在持股決策總覽的位置"><button type="button" class="position-reorder-button" data-position-move="up" data-position-code="'+e(r.code)+'" '+(positionIndex===0?'disabled ':'')+'aria-label="將 '+e(r.name)+' 上移">↑ <span>上移</span></button><button type="button" class="position-reorder-button" data-position-move="down" data-position-code="'+e(r.code)+'" '+(positionIndex===items.length-1?'disabled ':'')+'aria-label="將 '+e(r.name)+' 下移">↓ <span>下移</span></button></div>';
      return '<article class="position-card tone-'+e(view.key)+'" data-position-code="'+e(r.code)+'"><section><div class="position-stock-title" data-position-drag-handle="'+e(r.code)+'" title="按住滑鼠左鍵拖曳調整位置"><a class="stock-link" href="'+stockUrl(r.code)+'" target="_blank" rel="noreferrer" draggable="false"><b>'+e(r.code)+'</b> '+e(r.name)+'</a><span title="查看評分明細">'+scoreAnchor(r)+'</span>'+moveButtons+'</div><div class="position-metrics"><span><small>現價</small><b>'+n(price,2)+'</b></span><span><small>損益</small><b class="'+(Number.isFinite(pnl)&&pnl>=0?'state-add':'state-trim')+'">'+s(pnl,1,'%')+'</b></span></div>'+horizonMini(r)+'<label class="position-cost">成本<input type="number" min="0" step="0.01" value="'+e(Number.isFinite(cost)?cost:'')+'" data-position-cost="'+e(r.code)+'" aria-label="'+e(r.name)+' 成本價"></label><p class="position-card-note">中期排名 '+rankText+'</p><label class="tracking-toggle"><input type="checkbox" checked data-position-toggle="'+e(r.code)+'">持續追蹤</label></section><section class="position-action-pane"><small>目前狀態｜'+e(view.when)+'</small><span class="position-action-badge action-'+e(view.key)+'">'+e(view.label)+'</span><small>今天動作</small><strong>'+e(view.todayAction)+'</strong></section><section class="position-decision-pane"><div class="position-trigger-pane"><strong>下一次確認：'+e(view.nextCheck)+'</strong><p>'+e(view.instruction)+'</p>'+operationPriceHtml(p,r)+'<p>改變條件：'+e(view.change)+'</p></div><div class="position-reasons-pane"><strong>主要原因</strong><div class="position-reason-chips">'+reasons.map(reason=>'<span>'+e(reason.length>18?reason.slice(0,18)+'…':reason)+'</span>').join('')+'</div><p class="position-holding-plan"><b>持有計畫</b>'+e(r.holdingPlan)+'</p><details><summary>查看完整依據</summary><p><b>目前判斷：</b>'+e(basis)+'</p></details></div></section></article>'
  }).join('')
}
 function openScore(code,updateHash=true,initialTab='medium'){
  ...
  const healthDetails=[...(health.staleCore||[]),...(health.missingCore||[]),...(health.comparisonLimits||[]),...(health.freshnessLimits||[])];
  ...
  const detailRows=[
    ...
    componentRow('技術趨勢',c.technical,20,[
    ...
    componentRow('估值',c.valuation,15,[
      ...
      '本益比只在本項計分，不再用盈餘殖利率重複加分；低估值不直接等於便宜'
    ],'valuation',r.code),
    componentRow('ETF／外資／投信／自營商籌碼',c.ownership,10,[
    ...
    componentRow('事件、風險與流動性',c.eventRisk,10,[
      catalystEvents,
      ...
      '資料健康度不在本項計分；處置與事件硬門檻仍可直接覆蓋總分'
    ],'risk',r.code)
   ].join('');
   const shortComponents=h.short&&h.short.components||{};
   const longComponents=h.long&&h.long.components||{};
   const shortDetailRows=[
     componentRow('技術與時機',shortComponents.technicalTiming,30,['分析價 '+n(r.analysisPrice??r.livePrice??r.close,2)+'；5／20／60日EMA '+n(t.ema5,2)+'／'+n(t.ema20,2)+'／'+n(t.ema60,2),'RSI14 '+n(t.rsi14,1)+'；MACD、標準KD（9,3,3）與乖離共同判斷，不由單一交叉決定'],'technical',r.code),
     componentRow('ETF／法人短期流向',shortComponents.ownershipFlow,25,['ETF 5／10日 '+s(etf.d5,0,'張')+'／'+s(etf.d10,0,'張')+'；主動ETF 5日 '+s(etf.activeD5,0,'張'),'外資持股5／10日 '+s(foreign.holdingD5,0,'張')+'／'+s(foreign.holdingD10,0,'張')+'；外資買賣超5日 '+s(foreign.netBuy5,0,'張'),'投信買賣超5日 '+s(trust.netBuy5,0,'張')+'；只在趨勢資料可靠時解讀外資持股'],'ownership',r.code),
     componentRow('事件催化',shortComponents.eventCatalyst,15,[catalystEvents,'新聞屬待確認資訊，不直接計分；營收趨勢不在此重複計分'],'events_layer',r.code),
     componentRow('風險與流動性',shortComponents.riskLiquidity,15,['單日成交金額 '+moneyText(risk.dailyValue)+'；20日每日波動 '+n(risk.dailyVolatility20,2)+'%','最大單一ETF持股占比 '+n(risk.etfTop1Concentration,1)+'%；融資、借券與集保為獨立風險覆蓋'],'risk',r.code),
     componentRow('基本面護欄',shortComponents.fundamentalGuardrail,15,['單月營收年增 '+s(f.revenueYoy,1,'%')+'；季報 '+e(f.financialPeriod||'—')+'（'+e(financialModeText)+'）','基本面護欄用來避免時機訊號脫離公司現況，不建立另一套排名'],'fundamentals',r.code)
   ].join('');
   const longDetailRows=[
     componentRow('企業營運品質',longComponents.businessQuality,25,['季報 '+e(f.financialPeriod||'—')+'（'+e(financialModeText)+'）；毛利率／營業利益率／淨利率 '+n(f.grossMargin,1)+'%／'+n(f.operatingMargin,1)+'%／'+n(f.netMargin,1)+'%；EPS '+n(f.eps,2),'金融業與一般產業採不同獲利品質判斷；不含估值'],'fundamentals',r.code),
     componentRow('財務韌性',longComponents.financialResilience,20,['負債比 '+n(f.debtRatio,1)+'%；流動比 '+n(f.currentRatio,1)+'；EPS '+n(f.eps,2),'缺少可驗證財務資料時不以中性值補分'],'fundamentals',r.code),
     componentRow('成長耐久性',longComponents.growthDurability,15,['單月營收年增 '+s(f.revenueYoy,1,'%')+'；累計營收年增 '+s(f.revenueYtdYoy,1,'%'),'以趨勢與持續性作長期初篩，不代表獲利預測'],'fundamentals',r.code),
     componentRow('估值',longComponents.valuation,25,['本益比 '+n(v.pe,1)+' 倍；股價淨值比 '+n(v.pb,1)+' 倍；殖利率 '+n(v.yield,1)+'%','本益比只在估值構面計分，低估值不直接等於便宜'],'valuation',r.code),
     componentRow('資本配置品質',longComponents.capitalAllocation,15,['尚未計分：目前缺完整自由現金流、ROIC及資本配置紀律資料','此15分不顯示為0分；長期方法覆蓋固定85%，不可稱為完整長期價值評估'],'fundamentals',r.code)
   ].join('');
  ...
  const horizonNotes='<div class="score-horizon-note"><div><b>短期時機</b>回答現在是否適合觀察、等待或分批；不預測單日漲跌。</div><div><b>中期研究／排名主軸</b>回答未來數週至數月最值得先研究誰；本表下方只拆解這個分數。</div><div><b>長期初篩</b>方法覆蓋 '+n(h.long&&h.long.methodCoverage,0)+'%，本股資料覆蓋 '+n(h.long&&h.long.dataCoverage,0)+'%；缺完整自由現金流、ROIC與資本配置紀律，不單獨形成買賣動作。</div></div>';
   const detailTable=(label,rows)=>'<div class="score-detail-meta"><b>'+e(label)+'</b><span>構面實得／最高；證據、資料來源與改變條件列於下表</span></div><div class="score-detail-wrap"><table class="score-breakdown"><thead><tr><th>評估構面</th><th>實得／最高</th><th>本股證據</th><th>資料與消息來源</th></tr></thead><tbody>'+rows+'</tbody></table></div>';
   const tabIntro=(title,range,purpose,next,score,extra)=>'<div class="score-tab-intro"><div><b>'+e(title)+'</b><span>時間範圍：'+e(range)+'</span><span>用途：'+e(purpose)+'</span></div><strong>'+n(score,0)+'<small>/ 100</small></strong></div><div class="score-tab-facts"><span><b>下一次檢查</b>'+e(next)+'</span><span><b>改變條件</b>'+e(extra)+'</span></div>';
   scoreTabContent={short:tabIntro('短期時機','1–20個交易日（約1日到1個月）','只回答進場時機，不預測單日漲跌','每個交易日收盤後',h.short&&h.short.score,'收盤後技術、ETF／法人流向或事件狀態改變')+detailTable('短期五項構面',shortDetailRows),medium:tabIntro('中期研究／排名主軸','約1–6個月（約1–2個財報季度）','決定研究排序，不直接取代今天動作','月營收、季報或重大事件出現後',h.medium&&h.medium.score,'營收、季報、技術、估值、籌碼或事件風險證據更新')+detailTable('中期六項構面',detailRows),long:tabIntro('長期初篩','1年以上；價值投資宜觀察3–5年以上','只作企業品質與估值研究初篩，不是完整價值分數','每季財報及年度資本配置檢查',h.long&&h.long.score,'季報、年度資料或估值條件更新')+ '<div class="long-coverage-note"><b>方法覆蓋 '+n(h.long&&h.long.methodCoverage,0)+'%／資本配置品質尚未計分／15</b><span>本股資料覆蓋率 '+n(h.long&&h.long.dataCoverage,0)+'%；缺完整自由現金流、ROIC及資本配置紀律資料。</span></div>'+detailTable('長期五項構面',longDetailRows)};
   scoreDialogTitle.textContent=r.code+' '+r.name+'｜三時間尺度評分';scoreDialogSub.textContent=r.industry+' · 中期排名第 '+r.rank;
   scoreDialogBody.innerHTML='<div class="score-dates"><span>ETF '+e(reportMeta.etfDate)+'</span><span>法人買賣超 '+e(reportMeta.institutionalDate)+'</span><span>外資持股 '+e(reportMeta.foreignHoldingDate)+'</span><span>價量／估值 '+e(reportMeta.marketDate)+'</span></div><div class="score-summary"><div><span>短期時機</span><b>'+n(h.short&&h.short.score,0)+'</b></div><div class="is-primary"><span>中期研究／排名主軸</span><b>'+n(h.medium&&h.medium.score,0)+'</b></div><div><span>長期初篩</span><b>'+n(h.long&&h.long.score,0)+'</b></div><div><span>資料健康</span><b>'+n(health.score,0)+'% '+e(health.status||'')+(health.comparisonStatus==='部分可比'?'／部分可比':'')+'</b></div></div>'+healthNotice+'<div class="decision-strip"><span><small>尚未持有</small><b>'+e(r.entryAction)+'</b></span><span class="state-'+e(r.holdingState)+'"><small>已經持有</small><b>'+e(r.holdingAction)+'</b></span></div><div class="score-judgment"><b>今天動作：</b>'+e(r.todayAction||'依持股決策總覽確認')+'<br><b>下一次確認：</b>'+e(r.nextCheck||'每個交易日收盤後')+'</div><label class="position-toggle"><input type="checkbox" data-position-toggle="'+e(r.code)+'"> <span>我已開始布局，持續追蹤</span></label><div class="score-tabs" role="tablist" aria-label="時間尺度評分明細"><button type="button" role="tab" data-score-tab="short" aria-selected="false">短期時機</button><button type="button" role="tab" data-score-tab="medium" aria-selected="true" class="is-active">中期研究／排名主軸</button><button type="button" role="tab" data-score-tab="long" aria-selected="false">長期初篩</button></div><div id="scoreTabPanel" role="tabpanel"></div><div class="cross-horizon-reading"><b>跨時間尺度解讀（研究解讀，不覆蓋今天動作）</b><p>'+e(crossHorizonReading(h))+'</p></div>'+eventsHtml+'<div class="score-judgment"><b>已持有部位：</b>'+e(r.holdingPlan)+'<br><b>最先注意的風險：</b>'+e(risks)+'</div><p class="score-formula">三個頁籤只切換既有分數的解釋內容，不改變排名、建立新部位、已持有動作、今天動作或硬性門檻。資料健康度只判斷研究是否可採用，不進入任何分數或乘數。</p>';
   scoreTab(initialTab);
  scoreDialogTitle.innerHTML='<a class="stock-link" href="'+stockUrl(r.code,r.market)+'" target="_blank" rel="noreferrer" aria-label="開啟 '+e(r.code)+' '+e(r.name)+' Yahoo奇摩股市技術分析">'+e(r.code)+' '+e(r.name)+'</a>｜三時間尺度評分';
...
let syncTop30ScrollWidth=()=>{};let syncFullScrollWidth=()=>{};let scrollersInitialized=false;
 function draw(){const q=search.value.trim().toLowerCase();let filtered=rows.filter(r=>(!q||(r.code+' '+r.name).toLowerCase().includes(q))&&(!bucket.value||r.bucket===bucket.value)&&(!sector.value||r.sector===sector.value)&&(!positionOnly.checked||positions[r.code]));const state=tableSortState.full;if(state){filtered=[...filtered].sort((a,b)=>{const av=state.key==='rank'?a.rank:a.horizonScores&&a.horizonScores[state.key]?a.horizonScores[state.key].score:null;const bv=state.key==='rank'?b.rank:b.horizonScores&&b.horizonScores[state.key]?b.horizonScores[state.key].score:null;const diff=(Number.isFinite(av)?av:-Infinity)-(Number.isFinite(bv)?bv:-Infinity);return (state.direction==='asc'?diff:-diff)||a.rank-b.rank})}const rendered=filtered.map(r=>{const h=r.horizonScores||{};const health=r.dataHealth||{};return '<tr><td>'+r.rank+'</td><td><a class="stock-link" href="'+stockUrl(r.code)+'" target="_blank" rel="noreferrer" title="開啟 '+e(r.name)+' Yahoo技術分析"><b>'+e(r.code)+'</b> '+e(r.name)+'</a></td><td><input class="table-position-check" type="checkbox" data-position-toggle="'+e(r.code)+'" aria-label="追蹤 '+e(r.code)+' '+e(r.name)+'"></td><td>'+e(r.industry)+'</td><td class="tag-'+r.bucket+'">'+e(r.entryAction)+'</td><td class="state-'+e(r.holdingState)+'">'+e(r.holdingAction)+'</td><td>'+scoreAnchor(r,'short',h.short&&h.short.score)+'</td><td>'+scoreAnchor(r)+'</td><td>'+scoreAnchor(r,'long',h.long&&h.long.score)+'</td><td>'+n(health.score,0)+'% '+e(health.status||'')+(health.comparisonStatus==='部分可比'?'／部分可比':'')+'</td><td>'+n(r.livePrice??r.close,2)+'</td><td>'+s(r.fundamentals.revenueYoy,1,'%')+'</td><td>'+s(r.fundamentals.revenueYtdYoy,1,'%')+'</td><td>'+n(r.valuation.pe)+'</td><td>'+s(r.etf.d5)+'</td><td>'+s(r.etf.d10)+'</td><td>'+s(r.etf.activeD5)+'</td><td>'+n(r.foreign.holdingRatio,2)+'%</td><td>'+s(r.foreign.holdingD5)+'</td><td>'+s(r.foreign.holdingD10)+'</td><td>'+s(r.foreign.netBuy5)+'</td><td>'+s(r.investmentTrust.netBuy5)+'</td><td>'+s(r.investmentTrust.netBuy10)+'</td><td>'+n(r.technical.rsi14)+'</td><td>'+n(r.technical.kdK,1)+'／'+n(r.technical.kdD,1)+'</td><td>'+s(r.technical.distanceEma20,1,'%')+'</td><td>'+e((r.rejectionReasons||[]).join('；')||'—')+'</td></tr>'}).join('');body.innerHTML=rendered||'<tr><td class="empty-state" colspan="27"><b>目前沒有股票符合這組篩選條件。</b><br>只有「可開始承接」代表新部位通過全部門檻；既有部位請查看「我的布局追蹤」，不要因排名變化直接賣出。</td></tr>';syncPositionChecks();updateSortButtons('full');requestAnimationFrame(syncFullScrollWidth)}
 document.addEventListener('click',event=>{const move=event.target.closest('[data-position-move]');if(move){event.preventDefault();movePosition(move.dataset.positionCode,move.dataset.positionMove==='up'?-1:1);return}});
...
async function main() {
  ...
  let previousInternational;
  const priorPath = path.join(OUT_DIR, 'latest.json');
  if (fs.existsSync(priorPath)) previousInternational = JSON.parse(fs.readFileSync(priorPath, 'utf8')).internationalContext;
  const internationalPromise = startRequiredTask(() => fetchInternationalContext({ asOf: TODAY, previous: previousInternational }), 'international context');
  ...
  const reportableEvents = eventsLayerData.events.filter(ev => !PUBLIC_EXCLUDED_EVENT_TYPES.has(ev.eventType));
  const reportEventSourceScope = eventsLayerData.sourceScope ? {
    ...eventsLayerData.sourceScope,
    xiaoyuEventTypes: (eventsLayerData.sourceScope.xiaoyuEventTypes || [])
      .filter(type => !PUBLIC_EXCLUDED_EVENT_TYPES.has(type))
  } : null;
  const eventsByCode = new Map();
  for (const ev of reportableEvents) {
    if (ev.code) {
  ...
  const twseListedCodes = new Set([
    ...
    ...twseSecuritiesRows.map(row => String(row['公司代號'] || '').trim())
  ]);
  const tpexListedCodes = new Set([
    ...tpexDailyRows.map(row => String(row.SecuritiesCompanyCode || '').trim()),
    ...tpexSecuritiesRows.map(row => String(row['證券代號'] || '').trim())
  ]);
  const candidateStockEntries = allStockEntries.filter(([code]) => /^\d{4}$/.test(code));
  const marketByCode = new Map(candidateStockEntries.map(([code]) => [
    code,
    twseListedCodes.has(code) ? 'TWSE' : tpexListedCodes.has(code) ? 'TPEX' : 'UNKNOWN'
  ]));
  // The ETF snapshot also contains numeric codes from foreign holdings (for
  // example, the global ETF 009826). Keep them in the source universe for
  // auditability, but do not rank them as Taiwan-listed/OTC stocks without an
  // official TWSE or TPEX market match.
  const stockEntries = candidateStockEntries.filter(([code]) => ['TWSE', 'TPEX'].includes(marketByCode.get(code)));
  const staleEtfs = data.etfs.filter(etf => !etf.updated).map(etf => ({
    code: etf.code,
    name: etf.name,
    type: etf.type,
    date: etf.date || null
  }));
  const upstreamLaggingEtfCount = number(data.meta.lagging_etfs);
  if (Number.isFinite(upstreamLaggingEtfCount) && upstreamLaggingEtfCount !== staleEtfs.length) {
    throw new Error(`ETF source freshness metadata mismatch: lagging_etfs=${upstreamLaggingEtfCount}, actual=${staleEtfs.length}`);
  }
  ...
  const activeStaleEtfs = staleEtfs.filter(etf => etf.type === 'active').map(({ code, name, date }) => ({ code, name, date }));
  ...
  const laggingSet = new Set(staleEtfs.map(etf => etf.code));
  console.log(`ETF ${data.etfs.length} 檔，ETF持股共 ${allStockEntries.length} 檔，納入上市／上櫃普通股 ${stockEntries.length} 檔，主動ETF ${activeSet.size} 檔。`);

  console.log(`2/7 讀取${stockEntries.length}檔上市／上櫃普通股ETF快照與可用官方持股歷史...`);
  ...
  const twseEps = createLookup(twseEpsRows, ['公司代號', 'SecuritiesCompanyCode']);
  ...
  const tpexValuation = createLookup(tpexValuationRows, ['SecuritiesCompanyCode']);
  const tpexRevenue = createLookup(tpexRevenueRows, ['公司代號', 'SecuritiesCompanyCode']);
  const tpexEps = createLookup(tpexEpsRows, ['SecuritiesCompanyCode', '公司代號']);
  const tpexMargin = createLookup(tpexMarginRows, ['SecuritiesCompanyCode', '公司代號']);
  const tpexDaily = createLookup(tpexDailyRows, ['SecuritiesCompanyCode']);
  const tpexForeignHolding = createLookup(tpexForeignHoldingRows, ['SecuritiesCompanyCode']);
  const tpexCredit = createLookup(tpexCreditRows, ['SecuritiesCompanyCode']);
  const tpexBalance = createLookup(tpexBalancePayloads.flat(), ['公司代號', 'SecuritiesCompanyCode']);
  const tpexIncome = createLookup(tpexIncomePayloads.flat(), ['公司代號', 'SecuritiesCompanyCode']);
  const financialCurrentPeriod = [...incomePayloads.flat(), ...balancePayloads.flat(), ...twseEpsRows,
    ...tpexIncomePayloads.flat(), ...tpexBalancePayloads.flat(), ...tpexEpsRows]
  ...
  const records = stockEntries.map(([code, stock], index) => {
    ...
    const market = marketByCode.get(code) || 'UNKNOWN';
    const valuationRow = market === 'TPEX' ? tpexValuation.get(code) : twseValuation.get(code);
    const revenueRow = market === 'TPEX' ? tpexRevenue.get(code) : twseRevenue.get(code);
    const epsRow = market === 'TPEX' ? tpexEps.get(code) : twseEps.get(code);
    const dailyRow = market === 'TPEX' ? tpexDaily.get(code) : twseDaily.get(code);
    const balanceRow = market === 'TPEX' ? tpexBalance.get(code) : twseBalance.get(code);
    const incomeRow = market === 'TPEX' ? tpexIncome.get(code) : twseIncome.get(code);
    const marginRow = market === 'TPEX' ? tpexMargin.get(code) : twseMargin.get(code);
    ...
    const currentOpMargin = market === 'TWSE'
      ? number(marginRow?.['營業利益率(%)(營業利益)/(營業收入)'])
      : number(marginRow?.['營業利益率']);
    const opMargin = currentOpMargin ?? (usePriorFinancial ? priorFinancial.operatingMargin : null);
    const quarterRevenue = number(incomeRow?.['營業收入'] ?? incomeRow?.['收益']);
    const quarterGrossProfit = number(incomeRow?.['營業毛利（毛損）']);
    const quarterOperatingIncome = number(incomeRow?.['營業利益（損失）'] ?? incomeRow?.['營業利益']);
    const quarterPretaxIncome = number(incomeRow?.['稅前淨利（淨損）'] ?? incomeRow?.['稅前淨利']);
    const quarterParentNetIncome = number(incomeRow?.['淨利（淨損）歸屬於母公司業主'] ?? incomeRow?.['本期淨利（淨損）']);
    const currentGrossMargin = number(marginRow?.['毛利率(%)(營業毛利)/(營業收入)'] ?? marginRow?.['毛利率']) ?? (quarterRevenue ? quarterGrossProfit / quarterRevenue * 100 : null);
    const currentNetMargin = number(marginRow?.['稅後純益率(%)(稅後損益)/(營業收入)'] ?? marginRow?.['稅後純益率']) ?? (quarterRevenue ? quarterParentNetIncome / quarterRevenue * 100 : null);
    ...
    const foreignHolding = market === 'TPEX'
      ? tpexForeignHoldingFeatures(tpexForeignHolding.get(code), asOfIso)
      : foreignHoldingFeatures(foreignHoldingHistory, code);
    ...
    const credit = market === 'TPEX'
      ? tpexCreditFeatures(tpexCredit.get(code), asOfIso)
      : creditFeatures(creditHistory, code, technical?.return5, (number(dailyRow?.TradeVolume) || 0) / 1000);
  ...
  const marketUniverseCodes = new Set([
    ...twseDailyRows.map(row => String(row.Code || '').trim()),
    ...twseSecuritiesRows.map(row => String(row['公司代號'] || '').trim()),
    ...tpexDailyRows.map(row => String(row.SecuritiesCompanyCode || '').trim()),
    ...tpexSecuritiesRows.map(row => String(row['證券代號'] || '').trim())
  ].filter(code => /^\d{4}$/.test(code)));
  const report = {
    meta: {
      ...
      etfSourceGeneratedAt: data.meta.generated || null,
      ...
      institutionalSource: `${institutionalHistory.sourceMode} + TPEX official current snapshot`,
      ...
      listedUniverseCount: marketUniverseCodes.size,
      coverageRate: round(stockEntries.length / Math.max(1, allStockEntries.length) * 100, 1),
      allEtfHeldStocks: stockEntries.length,
      rawEtfHeldStocks: allStockEntries.length,
      taiwanEtfHeldStocks: stockEntries.length,
      twseHeldStockCount: stockEntries.filter(([code]) => marketByCode.get(code) === 'TWSE').length,
      tpexHeldStockCount: stockEntries.filter(([code]) => marketByCode.get(code) === 'TPEX').length,
      unknownMarketStockCount: candidateStockEntries.filter(([code]) => marketByCode.get(code) === 'UNKNOWN').length,
      ...
      activeStaleEtfs,
      etfUpdated: data.etfs.length - staleEtfs.length,
      staleEtfCount: staleEtfs.length,
      staleEtfs,
      laggingEtfs: staleEtfs.length,
      incomplete: data.meta.incomplete,
      deepScored: records.filter(record => record.dataHealth?.eligible).length,
      bucketA: records.filter(record => record.bucket === 'A').length,
      rejected: records.filter(record => record.bucket === 'D').length,
      scope: '涵蓋籌碼小宇ETF持股資料集中的全部可辨識四碼普通股，含上市與上櫃；TWSE／TPEX官方市場清單只確認市場別，ETF持股尚未逐檔與投信官方持股清單交叉核對；日期落後ETF的持有仍保留於母體避免漏股，但會形成個股ETF資料落後曝險，不能宣稱全體ETF持股皆為同日；上櫃股票的上市專屬官方欄位若無資料會保留缺漏，不以中性值補造'
    },
    macroOverlay,
    ...
    sourcePosture: {
      ...
      secondary: '籌碼小宇ETF持股與歷史價格、Yahoo Finance日K高低收屬次級資料；ETF持股尚未逐檔以投信官方清單對帳，個別ETF日期可能落後，須以資料健康度的ETF落後曝險解讀，法人買賣超只作官方資料的備援／交叉檢查',
      pending: 'Yahoo Finance RSS新聞只列待確認，不直接計分；日K高低收另列為B級技術輔助來源',
      scoreBoundary: '短、中、長期分數分開；資料健康度、宏觀、信用交易與集保資料屬獨立覆蓋，不重複灌入投資分數'
    },
    methodology: {
      modelVersion: 'HORIZON_SCORE_V2',
      primaryRankingHorizon: 'medium',
      horizonModels: HORIZON_MODELS,
      dataHealthPolicy: {
        hardGate: DATA_HEALTH_HARD_GATE,
        affectsScore: false,
        rule: '資料健康度只決定資料是否足以採用；不進入短、中、長期分數，也不再乘上信心度係數。'
      },
      antiDoubleCounting: [
        '本益比只在估值構面計分，不再以盈餘殖利率重複計分',
        '營收趨勢只在各時間尺度的單一基本面構面出現，不再放入事件催化構面',
        '資料健康度不進入風險分數、不乘分數，只保留獨立硬門檻'
      ],
      hierarchy: '看所有ETF持有的上市與上櫃普通股；ETF以20日看背景、10日看延續、5日看反轉；外資持股存量、外資買賣超、投信與自營商買賣超分開計算；KD只作進出場時機輔助，避免與RSI、MACD重複加權',
      hardFilters: ['資料健康度>=65%', '單日成交金額>=5,000萬元', '營收未同步明顯衰退', '中期趨勢未明顯向下', 'ETF 5/10日未同步減碼', '外資持股5/10日與外資買賣超未同步轉弱', '投信未持續且具金額意義地賣超', 'KD高低收資料完整且高檔死亡交叉僅作時機警示', '最終布局需具ETF加碼經濟金額或跨基金共識', '未處於處置風險'],
      caveat: '中期分數只用於研究排序；短期分數是時機，長期分數是資料尚不完整的初篩。未取得完整一致預估、自由現金流、ROIC、資本配置紀律、目標價與個人風險承受度，不是個人化投資建議。'
    },
    eventsMeta: reportableEvents.length > 0 ? {
      fetchedAt: eventsLayerData.fetchedAt,
      totalCount: reportableEvents.length,
      aiEnabled: eventsLayerData.aiEnabled,
      sourceScope: reportEventSourceScope,
      sourceStatus: eventsLayerData.sourceStatus,
      byType: reportableEvents.reduce((acc, ev) => {
    ...
    ranking,
    sources: {
      'Official TWSE security master for market classification': SOURCES.twseSecurities,
      'Official TPEX security master for market classification': SOURCES.tpexSecurities,
      '[A 官方原始] 證交所 T86 三大法人逐日買賣超': 'https://www.twse.com.tw/rwd/zh/fund/T86?response=html',
      '[A 官方] 證交所 OpenAPI 重大訊息': SOURCES.twseMaterialInfo,
      '[A 官方] 證交所 OpenAPI 上市季損益表': INCOME_ENDPOINTS.TWSE[0],
      ...
      '[B 次級] 籌碼小宇 ETF 持股（未逐檔官方對帳）': SOURCES.etf,
      '[B 次級] Yahoo Finance 日K高低收（技術／KD輔助）': `${SOURCES.yahooChart}{code}.TW?range=6mo&interval=1d`,
      '[C 待確認] Yahoo Finance RSS 新聞（不直接計分）': SOURCES.yahooFinanceRss,
      '[B 次級] 籌碼小宇 法人逐日買賣超整理（官方不足時備援）': SOURCES.institution,
      '[B 次級] 籌碼小宇 公司事件資料（庫藏股／處置）': SOURCES.events,
      ...
      '證交所 OpenAPI 上市月營收': SOURCES.twseRevenue,
      '證交所 OpenAPI 上市EPS': SOURCES.twseEps,
      '證交所 OpenAPI 上市資產負債表': BALANCE_ENDPOINTS.TWSE[0],
      '證交所 上市日行情': SOURCES.twseDaily,
      '證交所 即時行情': 'https://mis.twse.com.tw/stock/index.jsp',
      '櫃買中心 上櫃估值': SOURCES.tpexValuation,
      '櫃買中心 上櫃月營收': SOURCES.tpexRevenue,
      '櫃買中心 上櫃EPS': SOURCES.tpexEps,
      '櫃買中心 上櫃財務資料': BALANCE_ENDPOINTS.TPEX[0],
      '櫃買中心 上櫃日行情': SOURCES.tpexDaily,
      '櫃買中心 上櫃三大法人': SOURCES.tpexInstitutional,
      '櫃買中心 上櫃外資持股': SOURCES.tpexForeignHolding,
      '櫃買中心 上櫃融資融券': SOURCES.tpexMarginTrading,
      '證交所／櫃買中心 即時行情': 'https://mis.twse.com.tw/stock/index.jsp'
    },
    sourceGroups: {
      fundamentals: [
        { label: '證交所 OpenAPI 上市公司季損益表', url: INCOME_ENDPOINTS.TWSE[0] },
        { label: '證交所 OpenAPI 上市公司月營收', url: SOURCES.twseRevenue },
        { label: '證交所 OpenAPI 上市公司EPS', url: SOURCES.twseEps },
        { label: '證交所 OpenAPI 上市公司財務比率與資產負債資料', url: BALANCE_ENDPOINTS.TWSE[0] },
        { label: '櫃買中心上櫃公司季損益表', url: INCOME_ENDPOINTS.TPEX[0] },
        { label: '櫃買中心上櫃公司月營收', url: SOURCES.tpexRevenue },
        { label: '櫃買中心上櫃公司EPS', url: SOURCES.tpexEps },
        { label: '櫃買中心上櫃公司財務比率與資產負債資料', url: BALANCE_ENDPOINTS.TPEX[0] }
      ],
      valuation: [
        { label: '證交所本益比、股價淨值比與殖利率', url: SOURCES.twseValuation },
        { label: '櫃買中心上櫃本益比、股價淨值比與殖利率', url: SOURCES.tpexValuation }
      ],
      ownership: [
        ...
        { label: 'B級：籌碼小宇ETF持股（未逐檔官方對帳）', url: SOURCES.etf },
        { label: 'B級：籌碼小宇法人逐日買賣超整理（官方不足時備援）', url: SOURCES.institution },
        ...
        { label: '櫃買中心上櫃三大法人買賣明細', url: SOURCES.tpexInstitutional },
        { label: '櫃買中心上櫃外資持股比例', url: SOURCES.tpexForeignHolding }
      ],
      technical: [
        { label: 'B級：籌碼小宇個股價格序列（技術歷史）', url: `${XIAOYU}/data/stock/` },
        { label: 'B級：Yahoo Finance日K高低收（技術／標準KD輔助）', url: `${SOURCES.yahooChart}{code}.TW?range=6mo&interval=1d` },
        { label: '證交所每日行情', url: SOURCES.twseDaily },
        { label: '櫃買中心上櫃每日行情', url: SOURCES.tpexDaily },
      ...
      risk: [
        ...
        { label: '櫃買中心上櫃融資融券', url: SOURCES.tpexMarginTrading },
        { label: '集保股權分散表', url: SOURCES.tdccHoldingLevels },
        { label: '證交所每日行情與成交金額', url: SOURCES.twseDaily },
        { label: 'B級：籌碼小宇ETF持股與資料更新狀態', url: SOURCES.etf },
        { label: 'B級：籌碼小宇公司事件資料（庫藏股／處置）', url: SOURCES.events }
      ],
      events_layer: [
        { label: '證交所 OpenAPI 重大訊息（官方確認）', url: SOURCES.twseMaterialInfo },
        { label: 'C級：Yahoo Finance 個股新聞（待確認、不直接計分）', url: SOURCES.yahooFinanceRss },
        { label: 'B級：籌碼小宇公司事件資料（庫藏股／處置）', url: SOURCES.events }
      ]
    }
  };
...
function renderExistingReport() {
  ...
  if (report.meta.scoringModelVersion !== 'HORIZON_SCORE_V2' || report.methodology?.primaryRankingHorizon !== 'medium') {
    throw new Error('既有 latest.json 不是三時間尺度評分模型，必須執行完整受控更新，不得沿用舊分數重繪');
  }
