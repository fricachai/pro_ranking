...
只回報背景工作的 `RUN_LOG` 與 `STATUS=started`。完成結果必須使用 `/update-report-status` 查詢；不要猜測結果或以舊資料發布。不要預先讀取完整 `index.html` 或 `professional-screen-report/latest.json`，也不要修改評分公式、資料來源、驗證門檻或版面。腳本會強制重新抓取事件與新聞；任一必要來源、資料契約、新聞覆蓋率、Git 推送或 GitHub Pages 線上驗證失敗，都會停止且不得以舊資料發布。
