...
function sourceLinks(key,code){return '<div class="source-links">'+(sourceGroups[key]||[]).map(item=>{let url=item.url;if(key==='technical'&&url.endsWith('/data/stock/'))url+=encodeURIComponent(code)+'.json';url=url.replace('{code}',encodeURIComponent(code));return '<a href="'+e(url)+'" target="_blank" rel="noreferrer">'+e(item.label)+'</a>'}).join('')+'</div>'}
  ...
  function crossHorizonReading(h){const short=Number(h.short&&h.short.score);const medium=Number(h.medium&&h.medium.score);const long=Number(h.long&&h.long.score);const strong=value=>Number.isFinite(value)&&value>=65;const weak=value=>Number.isFinite(value)&&value<45;if(strong(short)&&weak(medium)&&weak(long))return '短期偏強、中長期偏弱：只代表短線反彈或時機，不是長期投資理由。';if(weak(short)&&strong(medium)&&strong(long))return '短期偏弱、中長期偏強：公司值得研究但進場時機不足，先等確認。';if(strong(short)&&strong(medium)&&weak(long))return '短中期偏強、長期偏弱：偏戰術或景氣循環研究，長期信心有限。';if(weak(short)&&weak(medium)&&strong(long))return '短中期偏弱、長期偏強：長期候選但目前週期不利，不急著建立部位。';return '方向大致一致／仍需依資料與動作門檻；三個尺度只提供研究解讀，不覆蓋今天動作。'}
