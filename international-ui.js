...
function renderInternationalContext(c) {
  ...
  const cards = [
     card('sp500', '美股 S&P 500', n(d.sp500?.value, 0), `20筆 ${s(d.sp500?.change20)}｜5筆 ${s(d.sp500?.change5)}`, d.sp500, '', '判斷全球風險偏好；走弱時先避免因排名追價'),
     card('fx', '美元／臺幣', n(d.fx?.usdTwd?.value, 3), `5筆 ${s(d.fx?.usdTwd?.change5)}｜上升＝臺幣貶值`, d.fx?.usdTwd, '期交所參考值，非銀行成交報價', '觀察外資與臺股資金壓力；貶值加劇時提高進場門檻'),
     card('treasury', '美國10年債殖利率', `${n(d.treasury?.value, 2)}%`, `5筆 ${s(Number.isFinite(d.treasury?.difference5) ? d.treasury.difference5 * 100 : null, 0, '基點')}｜2年 ${n(d.treasury?.y2, 2)}%`, d.treasury, '', '判斷估值壓力；上升時成長股需等待價格結構確認'),
     card('vix', 'VIX 波動指數', n(d.vix?.value, 2), '低於20／高於25為觀察門檻，非買賣訊號', d.vix, '', '判斷市場是否進入防守模式；高波動時以等待、不追價為主'),
     card('tx', '外資臺指期淨部位', `${n(d.tx?.net, 0)}口`, d.tx?.comparisonDate ? `較 ${d.tx.comparisonDate} ${s(d.tx.changeFromPrevious, 0, '口')}` : '首筆快照：尚無前期比較', null, '負值＝淨空；可能包含避險，不能單獨判空', '檢查法人情緒是否與個股價格同向；只作確認，不單獨決定買賣'),
     card('dollar', 'Fed 廣義美元', n(d.dollar?.value, 2), `公布期間 ${s(d.dollar?.periodChange)}｜${d.dollar?.periodStart || '—'} 起`, d.dollar, '非 ICE DXY；週資料有發布落差', '觀察全球資金與美元環境；變動時重新檢查匯率與外資條件')
  ].join('');
  ...
  return `<section class="section international-context" id="internationalContext" data-context-version="${e(c.version)}">
    ...
    <div class="context-refresh"><span>國際資料查詢：${e(localTime(c.checkedAt))}（臺北）${weak ? ` · ${weak}項落後／缺漏` : ''}</span><button id="checkPublishedUpdate" type="button">查看最新發布</button><span id="publishedUpdateStatus" role="status" aria-live="polite"></span></div>
     <details class="context-details"><summary>半導體、能源與國際法人部位</summary><div class="context-section-guide"><b>這一組回答：哪些產業或資金方向值得進一步核對？</b><span>它用來確認臺股個股的產業順風、成本壓力與法人情緒，不取代個股基本面，也不直接產生買賣訊號。</span></div><div class="context-grid context-grid-extra">
       ${card('sox', '費城半導體', n(d.sox?.value, 0), `5筆 ${s(d.sox?.change5)}｜相對S&P ${s(soxRelative, 1, '百分點')}`, d.sox, '', '確認電子／半導體個股是否有產業面配合；只作順風或逆風檢查')}
       ${card('oil', 'WTI近月期貨', `${n(d.oil?.value, 2)}美元／桶`, `5筆 ${s(d.oil?.change5)}｜注意合約轉倉`, d.oil, '', '檢查能源、運輸與製造成本壓力；先回看個股受益或受害方向')}
       ${card('fx', '亞洲匯率參考', `USD/JPY ${n(d.fx?.usdJpy?.value, 2)}`, `USD/CNY ${n(d.fx?.usdCny?.value, 4)}｜均為一美元兌本幣`, null, '', '補充出口、進口與區域資金背景；不能單獨判定臺股漲跌')}
