...
1. 所有需要「前期比較」的指標，必須從 `asOf` 往前選取最近一筆已結束且通過欄位驗證的營業日資料；週末、國定假日或盤後資料尚未形成時，不得把當日空白當成沒有歷史資料。
2. 時間序列共用 `seriesMetric` 的最近觀測與前一筆比較欄位；期交所外幣參考匯率保留官方歷史序列，外資臺指期淨部位使用期交所「區分各期貨契約／依日期」官方查詢頁逐日回補，至少取得最新觀測與前一營業日，才可呈現比較值。
3. 來源契約必須保存 `observedAt`、`comparisonDate`、`dateBackfillDays` 與必要的回補終點；畫面要顯示實際資料日期與比較日期／變化值，不得顯示「尚無前期比較」、「日期未知」、「資料不足」或同義的空泛拒答。
4. 若官方端點暫時失敗，依序使用官方日期回補、官方替代端點、第二個已驗證來源或前次已驗證快照；所有替代值都要標示實際日期與證據層級，不得補造數值。<!-- INTERNATIONAL_COMPARISON_BACKFILL_V1 -->
