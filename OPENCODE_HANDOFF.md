...
- `international-context.js` 原本只保存 Fed、EIA、OFAC 的標題、網址與日期。
- `international-ui.js` 對三類公告固定輸出「先不要因標題買進或賣出；先看原文與持股曝險」，沒有讀取官方公告內文，也沒有提供方向、傳導路徑或具體曝險檢查。

### 正在採用的方法

- 抓取每則官方公告原文，移除頁面導覽後保存可追溯的內文證據。
...
## 2026-09-21 Codex 閱讀導引與操作層級升級（已完成，已發布）
...
- 已完成發布：commit `e6f4836` 已推送 `origin/main`；Pages workflow `35602337919` 成功；正式網址 HTTP 200，線上首頁與本機 `index.html` SHA-256 完全一致，四步導引、`目前 0 檔可開始承接`、`19/22 檔主動ETF已核對` 與 `quick-controls` 標記均已確認。
