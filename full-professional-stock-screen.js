...
function classifyRecord(record) {
  ...
  const timingReject = reasons.some(reason => /改善不足|調節|1\/3日|主動ETF|投信|外資持股|尚未上彎|未守20日|過熱|高波動|急跌反轉|除權息|共識不足|估值高/.test(reason));
...
function buildHtml(report) {
  ...
  const topCards = report.topThree.map((row, index) => `
    <article class="pick">
      <div class="pick-head"><span class="rank">${index + 1}</span><div><h3><a class="stock-link" href="${yahooTechnicalUrl(row.code)}" target="_blank" rel="noreferrer" title="開啟 ${escapeHtml(row.name)} Yahoo技術分析">${escapeHtml(row.code)} ${escapeHtml(row.name)}</a></h3><p>${escapeHtml(row.industry)} · ${escapeHtml(row.action)}</p></div><strong>${scoreLink(row)}</strong></div>
...
@media(max-width:700px){.score-dialog-body{padding:14px 12px 20px}.score-dialog-head{padding:15px 12px}.score-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.score-summary div:nth-child(2){border-right:0}.score-summary div:nth-child(-n+2){border-bottom:1px solid var(--line)}}
</style>
<style>
...
<tr><td>15%</td><td>技術面與進場時機</td><td><ul><li>5／20／60日EMA與趨勢斜率：最高6.3分</li><li>RSI合理區間：最高3.6分</li><li>MACD動能變化：最高2.1分</li><li>距20日EMA乖離：最高3.0分</li></ul></td><td>用來判斷現在是否適合承接，不用來替代基本面。短線過熱、趨勢未上彎或跌破均線會改列等待。</td></tr>
...
function sourceLinks(key,code){return '<div class="source-links">'+(sourceGroups[key]||[]).map(item=>{let url=item.url;if(key==='technical'&&url.endsWith('/data/stock/'))url+=encodeURIComponent(code)+'.json';return '<a href="'+e(url)+'" target="_blank" rel="noreferrer">'+e(item.label)+'</a>'}).join('')+'</div>'}
...
function syncScrollWidth(){topScrollSizer.style.width=fullTable.scrollWidth+'px'}
function draw(){const q=search.value.trim().toLowerCase();const filtered=rows.filter(r=>(!q||(r.code+' '+r.name).toLowerCase().includes(q))&&(!bucket.value||r.bucket===bucket.value)&&(!sector.value||r.sector===sector.value));const rendered=filtered.map(r=>'<tr><td>'+r.rank+'</td><td><a class="stock-link" href="'+stockUrl(r.code)+'" target="_blank" rel="noreferrer" title="開啟 '+e(r.name)+' Yahoo技術分析"><b>'+e(r.code)+'</b> '+e(r.name)+'</a></td><td>'+e(r.industry)+'</td><td class="tag-'+r.bucket+'">'+e(r.action)+'</td><td>'+scoreAnchor(r)+'</td><td>'+n(r.confidence,0)+'%</td><td>'+n(r.livePrice??r.close,2)+'</td><td>'+s(r.fundamentals.revenueYoy,1,'%')+'</td><td>'+s(r.fundamentals.revenueYtdYoy,1,'%')+'</td><td>'+n(r.valuation.pe)+'</td><td>'+s(r.etf.d5)+'</td><td>'+s(r.etf.d10)+'</td><td>'+s(r.etf.activeD5)+'</td><td>'+n(r.foreign.holdingRatio,2)+'%</td><td>'+s(r.foreign.holdingD5)+'</td><td>'+s(r.foreign.holdingD10)+'</td><td>'+s(r.foreign.netBuy5)+'</td><td>'+s(r.investmentTrust.netBuy5)+'</td><td>'+s(r.investmentTrust.netBuy10)+'</td><td>'+n(r.technical.rsi14)+'</td><td>'+s(r.technical.distanceEma20,1,'%')+'</td><td>'+e((r.rejectionReasons||[]).join('；')||'—')+'</td></tr>').join('');body.innerHTML=rendered||'<tr><td class="empty-state" colspan="22"><b>目前沒有股票符合這組篩選條件。</b><br>若選的是 A 可分批布局，代表沒有個股同時通過全部基本面、籌碼、外資／投信、技術面與風險門檻；系統不會為了湊足三檔而降低標準。</td></tr>';requestAnimationFrame(syncScrollWidth)}
let scrollSync=false;topScroll.addEventListener('scroll',()=>{if(scrollSync)return;scrollSync=true;tableWrap.scrollLeft=topScroll.scrollLeft;requestAnimationFrame(()=>{scrollSync=false})});tableWrap.addEventListener('scroll',()=>{if(scrollSync)return;scrollSync=true;topScroll.scrollLeft=tableWrap.scrollLeft;requestAnimationFrame(()=>{scrollSync=false})});document.getElementById('scrollLeft').addEventListener('click',()=>tableWrap.scrollBy({left:-Math.max(320,tableWrap.clientWidth*.75),behavior:'smooth'}));document.getElementById('scrollRight').addEventListener('click',()=>tableWrap.scrollBy({left:Math.max(320,tableWrap.clientWidth*.75),behavior:'smooth'}));window.addEventListener('resize',syncScrollWidth);[search,bucket,sector].forEach(el=>el.addEventListener('input',draw));document.addEventListener('click',event=>{const link=event.target.closest('[data-score-code]');if(!link)return;event.preventDefault();openScore(link.dataset.scoreCode)});document.getElementById('scoreClose').addEventListener('click',()=>scoreDialog.close());scoreDialog.addEventListener('click',event=>{if(event.target===scoreDialog)scoreDialog.close()});scoreDialog.addEventListener('close',()=>{if(location.hash.startsWith('#score-'))history.replaceState(null,'',location.pathname+location.search)});draw();const initialScore=location.hash.startsWith('#score-')?decodeURIComponent(location.hash.slice(7)):'';if(initialScore)openScore(initialScore,false);
</script></body></html>`;
}
...
async function main() {
  ...
  const report = {
    ...
    methodology: {
      weights: WEIGHTS,
      hierarchy: '只看上市股票；ETF以20日看背景、10日看延續、5日看反轉；外資持股存量、外資買賣超、投信與自營商買賣超分開計算，避免重複加分',
